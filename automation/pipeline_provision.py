    ...
    return "bash -lc " + _shell_single_quote(
        "\n".join(
            [
                ...
                f"chmod 700 {shlex.quote(host_dir)}/wallet",
                f"mkdir -p {shlex.quote(identity_dir)}",
                "test -d " + shlex.quote(host_dir),
                "test -w " + shlex.quote(host_dir),
                "test -w " + shlex.quote(f"{host_dir}/wallet"),
                "if [ ! -s "
                ...
                "touch " + shlex.quote(f"{host_dir}/wallet/probe.txt"),
                "echo 'wallet_probe_test' > "
                + shlex.quote(f"{host_dir}/wallet/probe.txt")
                ,
                "grep -q 'wallet_probe_test' " + shlex.quote(f"{host_dir}/wallet/probe.txt"),
                "rm -f " + shlex.quote(f"{host_dir}/wallet/probe.txt"),
                # Persist probe result for evidence
                f'echo \'{{"wallet_probe":"success","path":"{host_dir}/wallet","mount_expected":"/app/data"}}\' > {host_dir}/wallet/wallet-prepared.json',
                f"echo 'wallet_probe_test' > {host_dir}/wallet/wallet-runtime.txt",
                f"grep -q 'wallet_probe_test' {host_dir}/wallet/wallet-runtime.txt",
                "cat > " + shlex.quote(conn_path) + " <<'JSON'",
                connection_json,
                "JSON",
                "jq -e . " + shlex.quote(conn_path),
...
def _compact_channel_convergence_rows(
        ...
        compacted.append(
            {
                ...
                "channel_id": str(row.get("channel_id", "")).strip().lower(),
                "target_peer": str(row.get("target_peer", "")).strip().lower(),
                "orderer_component": str(row.get("orderer_component", "")).strip(),
                "peer_joined": bool(row.get("peer_joined", False)),
                "orderer_present": bool(row.get("orderer_present", False)),
                "orderer_active": bool(row.get("orderer_active", False)),
    ...
    return sorted(
        ...
        key=lambda item: (
            str(item.get("host_id", "")),
            str(item.get("channel_id", "")),
            str(item.get("target_peer", "")),
            str(item.get("orderer_component", "")),
        ),
    )


def _channel_convergence_summary(rows: List[Dict[str, Any]]) -> Dict[str, int]:
    summary = {
        "rows": 0,
        "converged": 0,
        "not_converged": 0,
    ...
    for row in rows:
        ...
        if bool(row.get("converged", False)):
            ...
            summary["not_converged"] += 1
        if bool(row.get("peer_joined", False)):
            summary["peer_joined"] += 1
        if bool(row.get("orderer_active", False)):
            summary["orderer_active"] += 1
    return summary


def _is_orderer_tls_overwrite_command(command: str) -> bool:
...
def _resolve_runtime_template_for_entry(entry: Dict[str, Any]) -> Dict[str, Any]:
    ...
    elif component_type == "network_api":
    ...
    if component_type == "api_gateway":
        env["IDENTITY_CONFIG"] = "/app/data/identities.json"
        env["CONNECTION_PROFILE"] = "/app/data/connection.json"
        env["CCP_PATH"] = "/app/data/connection.json"
        env["WALLET_PATH"] = "/app/data/wallet"
...
def _ssh_command_for_plan_entry(entry: Dict[str, Any]) -> str:
    ...
    if _is_managed_runtime_component_type(component_type):
        ...
        def _with_api_runtime_guards(cmd: str, action_name: str) -> str:
            guarded = _with_bootstrap(cmd)
            if not (_is_api_runtime_component_type(component_type) and action_name == "start"):
                return guarded
            storage_mount = (
                template.get("storage_mount", {})
                if isinstance(template.get("storage_mount"), dict)
            ...
            expected_mount = str(storage_mount.get("container_path", "")).strip() or "/app/data"
            env_map = template.get("env", {}) if isinstance(template.get("env"), dict) else {}
            wallet_path = str(env_map.get("WALLET_PATH", "")).strip() or f"{expected_mount}/wallet"
            inspect_cmd = (
                f"docker container inspect {target} --format '{{{{range .Mounts}}}}{{{{.Destination}}}} {{{{end}}}}' "
                f"| grep -q {_shell_single_quote(expected_mount)}"
            )
            probe_cmd = (
                f"docker exec {target} /bin/sh -lc "
                + _shell_single_quote(
                    "set -e; "
                    f"test -d {shlex.quote(expected_mount)}; "
                    f"test -w {shlex.quote(expected_mount)}; "
                    f"mkdir -p {shlex.quote(wallet_path)}; "
                    f"echo wallet_probe_test > {shlex.quote(wallet_path)}/.wallet-rw-probe; "
                    f"grep -q wallet_probe_test {shlex.quote(wallet_path)}/.wallet-rw-probe"
                )
            )
            return f"{guarded} && {inspect_cmd} && {probe_cmd}"
        ...
        if action == "start":
            return _with_api_runtime_guards(f"docker container start {target}", "start")
...
def _postcheck_fabric_component(
    *,
    ...
    if _is_api_runtime_component_type(component_type):
        ...
        env_map = template.get("env", {}) if isinstance(template.get("env"), dict) else {}
        checks["api_data_mount_configured"] = (
            str(storage_mount.get("container_path", "")).strip() == "/app/data"
        )
        checks["wallet_path_configured"] = (
            str(env_map.get("WALLET_PATH", "")).strip() == "/app/data/wallet"
        )
    return {
    ...
    *,
    ...
    if peer_container:
        peers[peer_container] = {
            ...
            "tlsCACerts": {
                "path": "/app/data/msp/tlscacerts/tlsca-cert.pem"
            },
        }
...
def _api_gateway_identity_bootstrap_command(
    *,
    ...
    identities_payload = _api_gateway_identities_payload(
        ...
        cert_path=cert_path,
        key_path=key_path,
        ccp_path="/app/data/connection.json",
    )
...
def run_provision_stage(
    *,
    ...
    if state_store is not None:
        ...
        channel_convergence_report_path = (
            state_store.stage_artifacts_dir(run.run_id, "provision")
            / "channel-convergence-report.json"
        )
        ...
        if completed_checkpoint is not None:
            ...
            org_runtime_manifest_path = (
                state_store.stage_artifacts_dir(run.run_id, "provision")
                / "org-runtime-manifest.json"
            )
            runtime_bundle_manifest_path = (
                state_store.stage_artifacts_dir(run.run_id, "provision")
                / "runtime-bundle-manifest.json"
            ...
            persisted_org_runtime_manifest = {}
            ...
            persisted_channel_convergence_report = {}
            ...
            if org_runtime_manifest_path.exists():
                persisted_org_runtime_manifest = json.loads(
                    org_runtime_manifest_path.read_text(encoding="utf-8")
                )
                artifacts["org_runtime_manifest"] = str(org_runtime_manifest_path)
            ...
            if channel_convergence_report_path.exists():
                persisted_channel_convergence_report = json.loads(
                    channel_convergence_report_path.read_text(encoding="utf-8")
                )
                artifacts["channel_convergence_report"] = str(
                    channel_convergence_report_path
                )
            ...
            if persisted_org_runtime_manifest:
                if "org_runtime_manifest" not in provision_report:
                    provision_report[
                        "org_runtime_manifest"
                    ] = persisted_org_runtime_manifest
                if "org_runtime_manifest" not in runtime_inventory:
                    runtime_inventory[
                        "org_runtime_manifest"
                    ] = persisted_org_runtime_manifest
    ...
    org_runtime_manifest_payload = {
        "schema_version": "a2.6.org-runtime-manifest.v1",
        "run_id": run.run_id,
        "change_id": run.change_id,
        "manifest_fingerprint": str(manifest_report.get("manifest_fingerprint", "")).strip().lower(),
        "source_blueprint_fingerprint": str(manifest_report.get("source_blueprint_fingerprint", "")).strip().lower() or run.blueprint_fingerprint,
        "api_runtime_registry": {
            "manifest_org_id": str(manifest_org_id).strip().lower(),
            "orderer_names": [_api_runtime_first_orderer_name(manifest_report)] if _api_runtime_first_orderer_name(manifest_report) else [],
            "organizations": {
                str(manifest_org_id).strip().lower(): {
                    "org_id": str(manifest_org_id).strip().lower(),
                    "msp_id": f"{str(manifest_org_id).strip().lower()}MSP",
                    "peer_names": _api_runtime_peer_names_by_org(manifest_report).get(str(manifest_org_id).strip().lower(), []),
                    "scoped_channels": sorted(scoped_channels),
                    "declared_chaincodes": sorted({
                        str(item.get("chaincode_id", "")).strip().lower()
    ...
    org_runtime_manifest_payload["correlation"] = {
        "run_id": run.run_id,
        "change_id": run.change_id,
        "manifest_fingerprint": str(org_runtime_manifest_payload.get("manifest_fingerprint", "")).strip().lower(),
        "source_blueprint_fingerprint": str(org_runtime_manifest_payload.get("source_blueprint_fingerprint", "")).strip().lower(),
    }
    org_runtime_manifest_payload["runtime_contract_fingerprint"] = payload_sha256(org_runtime_manifest_payload)
            ...
            if ctype in API_RUNTIME_COMPONENT_TYPES:
                try:
                    bootstrap_payload = _materialize_api_runtime_bootstrap_for_entry(
                        ...
                        manifest_runtime_contract=org_runtime_manifest_payload,
                    )
                    ent["api_runtime_bootstrap"] = bootstrap_payload
                    identities_payload = (
                        bootstrap_payload.get("identities")
                        if isinstance(bootstrap_payload.get("identities"), dict)
                        else {}
                    )
                    organizations_payload = (
                        identities_payload.get("organizations")
                        if isinstance(identities_payload.get("organizations"), dict)
                        else {}
                    )
                    if not organizations_payload:
                        issues.append(
                            ProvisionIssue(
                                level="error",
                                code="provision_identity_generation_failed",
                                path=f"provision_execution_plan.entries[{idx}].api_runtime_bootstrap.identities.organizations",
                                message="Falha na geração de identities.json: nenhuma organização válida foi materializada.",
                            )
                        )
                    ...
                    for identity_org, identity_payload in sorted(organizations_payload.items()):
                        payload_dict = identity_payload if isinstance(identity_payload, dict) else {}
                        missing_fields = [
                            field
                            for field in required_identity_fields
                            if field not in payload_dict
                            or (
                                isinstance(payload_dict.get(field), str)
                                and not str(payload_dict.get(field)).strip()
                            )
                        ]
                        if missing_fields:
                            issues.append(
                                ProvisionIssue(
                                    level="error",
                                    code="provision_identity_incomplete",
                                    path=f"provision_execution_plan.entries[{idx}].api_runtime_bootstrap.identities.organizations.{identity_org}",
                                    message=f"Identidade incompleta para org '{identity_org}' (campos ausentes: {', '.join(missing_fields)}).",
                                )
                            )
                    if len(organizations_payload.keys()) > 1 and str(identities_payload.get("defaultOrg", "")).strip():
                        issues.append(
                            ProvisionIssue(
                                level="error",
                                code="provision_identity_default_org_ambiguous",
                                path=f"provision_execution_plan.entries[{idx}].api_runtime_bootstrap.identities.defaultOrg",
                                message="defaultOrg não pode ser publicado quando há múltiplas organizações válidas; o request deve informar 'org' ou header 'x-fabric-org'.",
                            )
                        )
                    _validate_api_runtime_ccp_contract(
                        ...
                        entry=ent,
                        environment_profile_ref=environment_profile_ref,
                        path_prefix=f"provision_execution_plan.entries[{idx}].api_runtime_bootstrap",
                        issues=issues,
                    )
                    ...
                    issues.append(
                        ProvisionIssue(
                            level="error",
                            code="provision_identity_generation_failed",
                            path=f"provision_execution_plan.entries[{idx}].api_runtime_bootstrap",
                            message=f"Falha ao materializar identities.json para runtime de API: {str(exc).strip() or 'erro não identificado'}",
                        )
                    )
    ...
    runtime_verify_rows: List[Dict[str, Any]] = []
    channel_convergence_rows: List[Dict[str, Any]] = []
    required_channel_convergence_count = 0
            ...
            for runtime_entry in _runtime_bootstrap_entries_for_host(
                ...
                if not runtime_ok:
                    ...
                    if declared_chaincode:
                        required_channel_convergence_count += 1
                        declared_channel = (
                            str(runtime_entry.get("channel_id", "")).strip().lower()
                        )
                        if not declared_channel:
                            issues.append(
                                ProvisionIssue(
                                    level="error",
                                    code="verify_channel_required_for_install",
                                    path=f"runtime_state.{host_ref}.runtime.{runtime_name}",
                                    message="Install Chaincodes bloqueado: runtime sem channel_id explícito.",
                                    runtime_name=runtime_name,
                                )
                            )
                            raise RuntimeError("channel-required-for-install")

                        orderer_component = (
                            str(
                                runtime_entry.get("orderer_component_id", "")
                                or runtime_entry.get("orderer_name", "")
                            )
                            .strip()
                            .lower()
                            or "orderer0"
                        )
                        orderer_convergence = _ensure_orderer_channel_participation(
                            run=run,
                            run_id=run.run_id,
                            host_id=host_ref,
                            orderer_component_id=orderer_component,
                            channel_name=declared_channel,
                            ssh_executor=resolved_ssh_executor,
                            state_store=state_store,
                            issues=issues,
                        )
                        peer_component = target_peer or runtime_name
                        peer_join = _ensure_peer_join_channel(
                            run=run,
                            run_id=run.run_id,
                            host_id=host_ref,
                            component_id=peer_component,
                            channel_name=declared_channel,
                            orderer_host=orderer_component,
                            orderer_port=7050,
                        ...
                        channel_converged = bool(
                            peer_join.get("joined", False)
                            and orderer_convergence.get("converged", False)
                        )
                        channel_convergence_rows.append(
                            {
                                "host_id": host_ref,
                                "runtime_name": runtime_name,
                                "channel_id": declared_channel,
                                "target_peer": peer_component,
                                "orderer_component": orderer_component,
                                "peer_joined": bool(peer_join.get("joined", False)),
                                "orderer_present": bool(orderer_convergence.get("present", False)),
                                "orderer_active": bool(orderer_convergence.get("active", False)),
                                "converged": channel_converged,
                            }
                        )
                        if not channel_converged:
                            issues.append(
                                ProvisionIssue(
                                    level="error",
                                    code="verify_channel_convergence_required",
                                    path=f"runtime_state.{host_ref}.runtime.{runtime_name}",
                                    message=(
                                        "Install Chaincodes bloqueado: canal não convergido "
                                        f"(channel='{declared_channel}', peer='{peer_component}', orderer='{orderer_component}')."
                                    ),
                                    runtime_name=runtime_name,
                                )
                            )
                            raise RuntimeError("channel-convergence-not-achieved")

                        if _is_orderer_tls_overwrite_command(runtime_command):
                            issues.append(
                                ProvisionIssue(
                                    level="error",
                                    code="orderer_tls_overwrite_detected",
                                    path=f"runtime_state.{host_ref}.runtime.{runtime_name}",
                                    message="Compose/entrypoint do orderer indica mutação de material TLS após convergência de canal.",
                                    runtime_name=runtime_name,
                                )
                            )
                            raise RuntimeError("orderer-tls-overwrite-detected")
    ...
    if required_channel_convergence_count > 0:
        converged_rows = [
            row for row in channel_convergence_rows if isinstance(row, dict)
        ]
        if len(converged_rows) < required_channel_convergence_count:
            issues.append(
                ProvisionIssue(
                    level="error",
                    code="verify_channel_convergence_inventory_missing",
                    path="runtime_inventory.channel_convergence.rows",
                    message=(
                        "Canal convergido ausente no inventário final para parte dos runtimes com lifecycle obrigatório."
                    ),
                )
            )
        for index, row in enumerate(converged_rows):
            if not bool(row.get("converged", False)):
                issues.append(
                    ProvisionIssue(
                        level="error",
                        code="verify_channel_convergence_required",
                        path=f"runtime_inventory.channel_convergence.rows[{index}]",
                        message="Canal não está ativo/convergido para execução de lifecycle.",
                        runtime_name=str(row.get("runtime_name", "")).strip(),
                    )
                )
    ...
    channel_convergence_rows = sorted(
        [dict(item) for item in channel_convergence_rows if isinstance(item, dict)],
        key=lambda item: (
            str(item.get("host_id", "")),
            str(item.get("channel_id", "")),
            str(item.get("target_peer", "")),
    ...
    channel_convergence_inline = {
        "run_id": run.run_id,
        "change_id": run.change_id,
        "stage": "provision",
        "summary": _channel_convergence_summary(channel_convergence_rows),
        "rows": _compact_channel_convergence_rows(channel_convergence_rows),
        "generated_at": str(execution_plan.get("generated_at", "")).strip()
        or run.started_at
        or utc_now_iso(),
    }
    channel_convergence_report = {
        **channel_convergence_inline,
        "inline": channel_convergence_inline,
        "rows_detailed": channel_convergence_rows,
    }
    ...
    channel_convergence_inline["correlation"] = artifact_correlation
    channel_convergence_report["correlation"] = artifact_correlation
    channel_convergence_report["inline"] = channel_convergence_inline
    ...
    verify_report_wp_a22 = {
        ...
        "summary": {
            ...
            "channel_convergence": dict(channel_convergence_inline.get("summary", {})),
        },
        "generated_at": utc_now_iso(),
        "correlation": artifact_correlation,
    }
    ...
    runtime_inventory = {
        ...
        "org_runtime_manifest": org_runtime_manifest_payload,
        ...
        "channel_convergence": channel_convergence_inline,
    ...
    if state_store is not None:
        ...
        org_runtime_manifest_bytes = json.dumps(
            org_runtime_manifest_payload,
            ensure_ascii=False,
            indent=2,
            sort_keys=True,
        ...
        channel_convergence_report_bytes = json.dumps(
            channel_convergence_report,
            ensure_ascii=False,
            indent=2,
            sort_keys=True,
        ...
        org_runtime_manifest_path = state_store.write_artifact(
            run_id=run.run_id,
            stage="provision",
            artifact_name="org-runtime-manifest.json",
            content=org_runtime_manifest_bytes,
        )
        for bundle_file in (
        ...
        channel_convergence_report_path = state_store.write_artifact(
            run_id=run.run_id,
            stage="provision",
            artifact_name="channel-convergence-report.json",
            content=channel_convergence_report_bytes,
        )
        ...
        artifacts = {
            ...
            "org_runtime_manifest": str(org_runtime_manifest_path),
            ...
            "channel_convergence_report": str(channel_convergence_report_path),
            ...
            input_hash=payload_sha256(
                {
                    ...
                    "org_runtime_manifest": {
                        "runtime_contract_fingerprint": str(
                            org_runtime_manifest_payload.get("runtime_contract_fingerprint", "")
                        ).strip().lower(),
                        "manifest_fingerprint": str(
                            org_runtime_manifest_payload.get("manifest_fingerprint", "")
                        ).strip().lower(),
                        "source_blueprint_fingerprint": str(
                            org_runtime_manifest_payload.get("source_blueprint_fingerprint", "")
                        ).strip().lower(),
                        "api_runtime_registry": dict(
                            org_runtime_manifest_payload.get("api_runtime_registry", {})
                        ),
                    },
                    "chaincode_runtime_bundle": {
                        "runtime_bundle_fingerprint": str(
    ...
    try:
        ...
        try:
            state_store.write_text_artifact(
                run_id,
                "provision",
                f"gateway-{host_id}-{component_id}-channel-list-{channel_name}.txt",
                list_out,
            )
        except Exception:
            pass
        ...
        if "Saved to" in fetch_out or "Saved" in fetch_out:
            ...
            block_available = "EXISTS" in str(getattr(blockcheck, "stdout", "") or "")
        else:
            ...
            if "Saved to" in participation_out or "Saved" in participation_out:
                ...
                block_available = "EXISTS" in str(getattr(blockcheck_after, "stdout", "") or "")

        if block_available and not joined:
            ...
            if "join successful" in join_out.lower() or "joined" in join_out.lower():
                joined = True
                try:
                    state_store.write_text_artifact(
                        run_id,
        ...
        if "CA_FOUND" in ca_out:
            ...
            issues.append(
                ProvisionIssue(
                    ...
                    code="msp_ca_invalid",
                    path=f"runtime_state.{host_id}.components.{component_id}",
                    message="MSP TLS CA ausente/inválida para remediação de join.",
                    runtime_name=component_id,
                )
            )

        # Orderer consenter evidence (best-effort)
        try:
            ...
            if "consenter joined" in consenter_out.lower():
                try:
                    state_store.write_text_artifact(
                        run_id,
                        "provision",
                        f"gateway-{host_id}-{component_id}-orderer-consenter-join-{channel_name}.txt",
                        consenter_out,
                    )
                except Exception:
                    pass
            ...
            elif "unknown authority" in consenter_out.lower() or "x509" in consenter_out.lower():
                issues.append(
                    ProvisionIssue(
                        level="error",
                        code="consenter_tls_unknown_authority",
                        path=f"runtime_state.{host_id}.components.{component_id}",
                        message="Falha TLS no consenter join (unknown authority).",
                        runtime_name=component_id,
                    )
                )
        except Exception:
        ...
        list_after = ssh_executor.execute_unit_with_output(
            run=run,
            host_id=host_id,
            component_id=component_id,
            operation=f"peer-channel-list-after:{host_id}:{component_id}",
            idempotency_key=f"peer-channel-list-after:{run_id}:{host_id}:{component_id}",
            command=f"peer channel list --peer={component_id} --channel={channel_name}",
        )
        list_after_out = str(getattr(list_after, "stdout", "") or "")
        try:
            state_store.write_text_artifact(
                run_id,
                "provision",
                f"gateway-{host_id}-{component_id}-channel-list-after-{channel_name}.txt",
                list_after_out,
            )
        except Exception:
            pass

        final_joined = bool(joined or (channel_name in list_after_out))
        if not final_joined:
            issues.append(
                ProvisionIssue(
                    level="error",
        ...
        return {
            ...
            "joined": final_joined,
            "channel_list_before_contains": bool(channel_name in list_out),
            "channel_list_after_contains": bool(channel_name in list_after_out),
        }
    except Exception as exc:
        issues.append(
            ProvisionIssue(
        ...
        return {
            ...
            "channel_id": channel_name,
            "joined": False,
            "channel_list_before_contains": False,
            "channel_list_after_contains": False,
        }


def _ensure_orderer_channel_participation(
    *,
    run: PipelineRun,
    run_id: str,
    host_id: str,
    orderer_component_id: str,
    channel_name: str,
    ssh_executor: Any,
    state_store: PipelineStateStore,
    issues: List[ProvisionIssue],
    ...
    try:
        list_before = ssh_executor.execute_unit_with_output(
            run=run,
            host_id=host_id,
            component_id=orderer_component_id,
            operation=f"orderer_channel_list:{host_id}:{orderer_component_id}:{channel_name}",
            idempotency_key=f"orderer_channel_list:{run_id}:{host_id}:{orderer_component_id}:{channel_name}",
            command=f"orderer channel list --orderer={orderer_component_id} --channel={channel_name}",
        )
        list_before_out = str(getattr(list_before, "stdout", "") or "")
        state_store.write_text_artifact(
            run_id,
            "provision",
            f"gateway-{host_id}-{orderer_component_id}-orderer-channel-list-{channel_name}.txt",
            list_before_out,
        )

        status_before = ssh_executor.execute_unit_with_output(
            run=run,
            host_id=host_id,
            component_id=orderer_component_id,
            operation=f"orderer_participation_status:{host_id}:{orderer_component_id}:{channel_name}",
            idempotency_key=f"orderer_participation_status:{run_id}:{host_id}:{orderer_component_id}:{channel_name}",
            command=f"orderer channel participation status --orderer={orderer_component_id} --channel={channel_name}",
        )
        status_before_out = str(getattr(status_before, "stdout", "") or "")
        state_store.write_text_artifact(
            run_id,
            "provision",
            f"gateway-{host_id}-{orderer_component_id}-orderer-participation-status-{channel_name}.txt",
            status_before_out,
        )

        present_before = channel_name in list_before_out
        if not present_before:
            ssh_executor.execute_unit_with_output(
                run=run,
                host_id=host_id,
                component_id=orderer_component_id,
                operation=f"orderer_channel_ensure:{host_id}:{orderer_component_id}:{channel_name}",
                idempotency_key=f"orderer_channel_ensure:{run_id}:{host_id}:{orderer_component_id}:{channel_name}",
                command=f"orderer channel participation ensure --orderer={orderer_component_id} --channel={channel_name}",
            )
        ...
        list_after = ssh_executor.execute_unit_with_output(
            ...
            host_id=host_id,
            component_id=orderer_component_id,
            operation=f"orderer_channel_list_after:{host_id}:{orderer_component_id}:{channel_name}",
            idempotency_key=f"orderer_channel_list_after:{run_id}:{host_id}:{orderer_component_id}:{channel_name}",
            command=f"orderer channel list --orderer={orderer_component_id} --channel={channel_name}",
        )
        list_after_out = str(getattr(list_after, "stdout", "") or "")

        status_after = ssh_executor.execute_unit_with_output(
            run=run,
            host_id=host_id,
            component_id=orderer_component_id,
            operation=f"orderer_participation_status_after:{host_id}:{orderer_component_id}:{channel_name}",
            idempotency_key=f"orderer_participation_status_after:{run_id}:{host_id}:{orderer_component_id}:{channel_name}",
            command=f"orderer channel participation status --orderer={orderer_component_id} --channel={channel_name}",
        )
        status_after_out = str(getattr(status_after, "stdout", "") or "")

        state_store.write_text_artifact(
            run_id,
            "provision",
            f"gateway-{host_id}-{orderer_component_id}-orderer-channel-list-after-{channel_name}.txt",
            list_after_out,
        )
        state_store.write_text_artifact(
            run_id,
            "provision",
            f"gateway-{host_id}-{orderer_component_id}-orderer-participation-status-after-{channel_name}.txt",
            status_after_out,
        )

        present = bool(channel_name in list_after_out) or present_before
        active = bool(channel_name in status_after_out) or ("active" in status_after_out.lower())
        converged = bool(present and active)
        if converged:
            issues.append(
                ProvisionIssue(
                    level="info",
                    code="verify_orderer_channel_converged",
                    path=f"runtime_state.{host_id}.components.{orderer_component_id}",
                    message="orderer channel participation converged",
                    runtime_name=orderer_component_id,
                )
            )
        else:
            issues.append(
                ProvisionIssue(
                    level="error",
                    code="verify_orderer_channel_not_converged",
                    path=f"runtime_state.{host_id}.components.{orderer_component_id}",
                    message="orderer channel participation not converged for required channel",
                    runtime_name=orderer_component_id,
                )
            )
        return {
        ...
        issues.append(
            ProvisionIssue(
                ...
                code="verify_orderer_channel_not_converged",
                path=f"runtime_state.{host_id}.components.{orderer_component_id}",
                message=str(exc).strip() or "orderer channel convergence failed",
                runtime_name=orderer_component_id,
            )
        )
        return {
...
def _validate_api_runtime_ccp_contract(
    *,
    ...
    ccp_payload = bootstrap_payload.get("connection_profile")
    if not isinstance(ccp_payload, dict):
        ccp_payload = {}

    identities_payload = bootstrap_payload.get("identities")
    if not isinstance(identities_payload, dict):
        identities_payload = {}
    organizations_payload = identities_payload.get("organizations")
    if not isinstance(organizations_payload, dict):
        organizations_payload = {}

    try:
        json.loads(json.dumps(ccp_payload, ensure_ascii=False, sort_keys=True))
    except Exception as exc:
        issues.append(
            ProvisionIssue(
                level="error",
                code="ccp_invalid",
                path=f"{path_prefix}.connection_profile",
                message=(
                    "connection.json inválido para runtime de API: "
                    f"{str(exc).strip() or 'erro de serialização'}"
                ),
            )
        )
        return

    channels_payload = ccp_payload.get("channels")
    ...
    for channel_id in expected_channels:
        if channel_id not in channels_payload:
            issues.append(
                ProvisionIssue(
                    level="error",
                    code="ccp_invalid",
                    path=f"{path_prefix}.connection_profile.channels",
                    message=f"Canal '{channel_id}' ausente no connection.json.",
                )
            )

    peers_payload = ccp_payload.get("peers")
    if not isinstance(peers_payload, dict):
        peers_payload = {}
    orderers_payload = ccp_payload.get("orderers")
    if not isinstance(orderers_payload, dict):
        orderers_payload = {}
    if not peers_payload or not orderers_payload:
        issues.append(
            ProvisionIssue(
                level="error",
                code="ccp_unreachable",
                path=f"{path_prefix}.connection_profile",
                message="connection.json sem peers/orderers suficientes para conexão Fabric.",
            )
        )

    def _validate_nodes(node_type: str, payload_map: Dict[str, Any]) -> None:
        for node_name, node_payload in sorted(payload_map.items(), key=lambda item: str(item[0])):
            details = node_payload if isinstance(node_payload, dict) else {}
            node_url = str(details.get("url", "")).strip()
            if not node_url:
                issues.append(
                    ProvisionIssue(
                        level="error",
                        code="ccp_unreachable",
                        path=f"{path_prefix}.connection_profile.{node_type}.{node_name}",
                        message=f"Endpoint ausente para {node_type[:-1]} '{node_name}'.",
                    )
                )
                continue
            if not node_url.startswith("grpcs://"):
                issues.append(
                    ProvisionIssue(
                        level="error",
                        code="ccp_tls_mismatch",
                        path=f"{path_prefix}.connection_profile.{node_type}.{node_name}.url",
                        message=f"Endpoint TLS inválido para {node_type[:-1]} '{node_name}' (esperado grpcs://).",
                    )
                )
            tls_payload = details.get("tlsCACerts")
            if not isinstance(tls_payload, dict):
                tls_payload = {}
            if not str(tls_payload.get("path", "")).strip():
                issues.append(
                    ProvisionIssue(
                        level="error",
                        code="ccp_tls_mismatch",
                        path=f"{path_prefix}.connection_profile.{node_type}.{node_name}.tlsCACerts.path",
                        message=f"TLS root ausente para {node_type[:-1]} '{node_name}'.",
                    )
                )

    _validate_nodes("peers", peers_payload)
    _validate_nodes("orderers", orderers_payload)

    is_external_linux = "external" in str(environment_profile_ref).lower() and "linux" in str(environment_profile_ref).lower()
    if is_external_linux:
        for org_name, org_payload in sorted(organizations_payload.items(), key=lambda item: str(item[0])):
            payload_dict = org_payload if isinstance(org_payload, dict) else {}
            if bool(payload_dict.get("discoveryAsLocalhost", False)):
                issues.append(
                    ProvisionIssue(
                        level="error",
                        code="ccp_invalid",
                        path=f"{path_prefix}.identities.organizations.{org_name}.discoveryAsLocalhost",
                        message="discoveryAsLocalhost deve ser false para external-linux.",
                    )
                )

    raw_context = entry.get("service_context") if isinstance(entry.get("service_context"), dict) else {}
    discovery_error = str(raw_context.get("discovery_error", "")).strip()
    if discovery_error and "access denied" in discovery_error.lower():
        bootstrap_payload["discovery_resolution"] = {
        ...
        issues.append(
            ProvisionIssue(
                level="warning",
                code="ccp_discovery_access_denied_fallback",
                path=f"{path_prefix}.discovery_resolution",
                message="Discovery ACL negada; fallback estático aplicado e registrado.",
            )
        )
    ...
    *,
    ...
    try:
        ...
        issues.append(
            ProvisionIssue(
                level="error",
                code="tls_san_missing",
                path=f"runtime_state.{host_id}.components.{component_id}",
                message="TLS SAN ausente no certificado do endpoint Fabric.",
                runtime_name=component_id,
            )
        )
...
# A2.6_RUNTIME_CONTRACT_OVERRIDE
@dataclass(frozen=True)
class ProvisionIssue:
    level: str
    code: str
    path: str
    message: str
    ...
    def to_dict(self) -> Dict[str, str]:
        ...
        runtime_name = str(self.runtime_name).strip()
        if runtime_name:
...
def _canonical_component_type(component_type: str) -> str:
    raw = str(component_type).strip().lower()
    if not raw:
        return ""
    normalized = re.sub(r"[^a-z0-9_\-]", "", raw)
    aliases = _COMPONENT_TYPE_ALIASES if isinstance(_COMPONENT_TYPE_ALIASES, dict) else {}
...
def _materialize_api_runtime_bootstrap_for_entry(
    *,
    ...
    default_peer_name: str,
    default_orderer_name: str,
    manifest_runtime_contract: Optional[Dict[str, Any]] = None,
) -> Dict[str, Any]:
    service_context = _normalized_service_context(entry.get("service_context", {}))
    org_id = _api_runtime_effective_org_id(
        service_context=service_context, manifest_org_id=manifest_org_id
    ...
    runtime_contract = (
        manifest_runtime_contract if isinstance(manifest_runtime_contract, dict) else {}
    )
    runtime_registry = (
        runtime_contract.get("api_runtime_registry")
        if isinstance(runtime_contract.get("api_runtime_registry"), dict)
        else {}
    )
    organizations = (
        runtime_registry.get("organizations")
        if isinstance(runtime_registry.get("organizations"), dict)
        else {}
    )
    org_contract = (
        organizations.get(org_id) if isinstance(organizations.get(org_id), dict) else {}
    )
    ...
    if not channel_ids and isinstance(org_contract.get("scoped_channels"), list):
        channel_ids = [
            str(c).strip().lower()
            for c in org_contract.get("scoped_channels", [])
            if str(c).strip()
        ]
    if not channel_ids and scoped_channels:
        channel_ids = sorted(scoped_channels)
    ...
    if not chaincode_ids and isinstance(org_contract.get("declared_chaincodes"), list):
        chaincode_ids = [
            str(c).strip().lower()
            for c in org_contract.get("declared_chaincodes", [])
            if str(c).strip()
        ]
    if not chaincode_ids and declared_chaincodes:
        chaincode_ids = sorted(declared_chaincodes)
    ...
    peer_name = ""
    if isinstance(org_contract.get("peer_names"), list) and org_contract.get("peer_names"):
        peer_name = str(org_contract.get("peer_names", [""])[0]).strip()
    if not peer_name and org_id in peer_names_by_org and peer_names_by_org[org_id]:
        peer_name = peer_names_by_org[org_id][0]
    if not peer_name:
        peer_name = default_peer_name or ""

    orderer_name = ""
    orderer_names = runtime_registry.get("orderer_names")
    if isinstance(orderer_names, list) and orderer_names:
        orderer_name = str(orderer_names[0]).strip()
    if not orderer_name:
        orderer_name = default_orderer_name or ""

    msp_id = str(org_contract.get("msp_id", "")).strip() or f"{org_id}MSP"
    ...
    connection_profile: Dict[str, Any] = {
        ...
        "client": {"organization": org_id},
        "organizations": {
            org_id: {"mspid": msp_id, "peers": [peer_name] if peer_name else []}
        },
        "peers": {},
        "orderers": {},
        "channels": {},
    ...
    for channel_id in channel_ids:
        connection_profile["channels"][channel_id] = (
            {"peers": {peer_name: {}}} if peer_name else {"peers": {}}
        )
    ...
    if isinstance(organizations, dict) and organizations:
        for raw_org_key, raw_org_contract in sorted(
            organizations.items(), key=lambda item: str(item[0]).strip().lower()
        ):
            normalized_org = str(raw_org_key).strip().lower()
            if not normalized_org:
                continue
            org_payload = raw_org_contract if isinstance(raw_org_contract, dict) else {}
            org_msp = str(org_payload.get("msp_id", "")).strip() or f"{normalized_org}MSP"
            org_channels = [
                str(c).strip().lower()
                for c in (org_payload.get("scoped_channels") or [])
                if str(c).strip()
            ]
            if not org_channels and normalized_org == org_id:
                org_channels = list(channel_ids)
            org_chaincodes = [
                str(c).strip().lower()
                for c in (org_payload.get("declared_chaincodes") or [])
                if str(c).strip()
            ]
            if not org_chaincodes and normalized_org == org_id:
                org_chaincodes = list(chaincode_ids)
            identities_by_org[normalized_org] = {
                "mspId": org_msp,
                "ccpPath": ccp_path,
                "connectionProfilePath": ccp_path,
                "certPath": f"{runtime_dir}/identities/{normalized_org}/cert.pem",
                "keyPath": f"{runtime_dir}/identities/{normalized_org}/key.pem",
                "channels": sorted(set(org_channels)),
                "chaincodes": sorted(set(org_chaincodes)),
                "discoveryAsLocalhost": bool(service_context.get("discoveryAsLocalhost", False)),
            }
    if not identities_by_org:
        identities_by_org[org_id] = dict(identity_entry)

    valid_orgs = sorted(identities_by_org.keys())
    identities = {
        "organizations": {
            org_key: dict(org_payload)
            for org_key, org_payload in sorted(identities_by_org.items(), key=lambda item: item[0])
        },
        "selectionContract": {
            "queryParam": "org",
            "header": "x-fabric-org",
            "requireExplicitOrgWhenAmbiguous": True,
            "validOrgs": valid_orgs,
        },
    }
    if len(valid_orgs) == 1:
        identities["defaultOrg"] = valid_orgs[0]
        identities["default"] = dict(identities_by_org[valid_orgs[0]])
    for normalized_org, normalized_payload in sorted(identities_by_org.items()):
        identities[normalized_org] = dict(normalized_payload)
    ...
    correlation = runtime_contract.get("correlation") if isinstance(runtime_contract, dict) else {}
    manifest_fingerprint = (
        str(correlation.get("manifest_fingerprint", "")).strip().lower()
    ...
    return {
        ...
        "source_contract": "org-runtime-manifest" if runtime_contract else "runtime-derived",
        "manifest_fingerprint": manifest_fingerprint,
        "bootstrap_fingerprint": fingerprint,
    }
