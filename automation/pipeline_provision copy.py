        ...
        *,
        ...
        try:
            ...
            try:
                ...
                try:
                    state_store.write_text_artifact(run.run_id, "provision", f"gateway-volume-mount-report.txt", volume_report)
                except Exception:
                    pass
            except Exception:
            ...
            captured = ssh_executor.execute_unit_with_output(
                ...
                idempotency_key=build_ssh_unit_idempotency_key(run=run, host_id=host_id, component_id=component_id, operation=f"probe_identities:{container_name}", component_signature=container_name),
                command=cmd,
                timeout_seconds=15.0,
            )
...
def _append_tech_debt_entry(state_store: Optional[PipelineStateStore], run: PipelineRun, entry: Dict[str, Any]) -> None:
    ...
    try:
        ...
        state_store.append_json_array_artifact(run.run_id, "provision", "workarounds-a2-tech-debt.json", payload)
    except Exception:
...
def _controlled_restart_container(
    *,
    ...
    try:
        try:
            idempotency = build_ssh_unit_idempotency_key(run=run, host_id=host_id, component_id=component_id, operation=f"controlled_restart:{container_name}", component_signature=container_name)
        except Exception:
            idempotency = hashlib.sha256(f"{run.run_id}|{host_id}|{component_id}|controlled_restart|{container_name}".encode("utf-8")).hexdigest()
        ...
        cmd = (
            f"/bin/sh -lc 'if [ -f \"{watch_path}\" ]; then docker inspect --format ""{{{{.State.Running}}}}"" {container_name} >/dev/null 2>&1 && docker restart -t 5 {container_name} || true; echo identities_reloaded; else echo identities_missing; exit 2; fi'"
        )
        ...
        _append_tech_debt_entry(state_store, run, {**artifact_payload, "action": "controlled_restart"})
        # persist raw SSH artifact as well
        try:
            state_store.write_text_artifact(run.run_id, "provision", f"gateway-{host_id}-{component_id}-controlled-restart-{container_name}.txt", stdout + "\n" + stderr)
        except Exception:
            pass
        return stdout
    except Exception:
        _append_tech_debt_entry(state_store, run, {"host_id": host_id, "component_id": component_id, "container": container_name, "reason": reason, "action": "controlled_restart_failed"})
        return None
...
def _diagnostic_guidance_for_issue(*, code: str, level: str) -> Tuple[str, str]:
    ...
    if normalized_code.startswith("provision_msp_tls_") or normalized_code.startswith("provision_identity_"):
        return (
    ...
    if normalized_code.startswith("provision_ca_tls_") or normalized_code == "provision_private_key_storage_not_authorized":
        return (
...
def _actionable_issue_payload(issue: ProvisionIssue, *, stage: str) -> Dict[str, str]:
    impact, action_recommended = _diagnostic_guidance_for_issue(code=issue.code, level=issue.level)
    ...
    runtime_name = str(issue.runtime_name).strip() or _runtime_name_from_issue_path(issue.path)
...
def _runtime_issue_catalog_payload() -> Dict[str, Any]:
    return {
        ...
        "issues": [
            {
                "code": code,
                "severity": str(entry.get("severity", "error")).strip().lower() or "error",
            }
            for code, entry in sorted(RUNTIME_BOOTSTRAP_ISSUE_CATALOG.items(), key=lambda item: item[0])
        ],
    }


def _runtime_plan_filter_issues(chaincode_runtime_plan: Dict[str, Any]) -> List[ProvisionIssue]:
    filtered_entries = (
...
def _append_provision_flow_checkpoint(
    *,
    ...
    checkpoint_payload = {
        ...
        "causa": str(cause).strip() or (
            f"Etapa '{normalized_stage}' finalizada com status '{normalized_status}'."
        ),
...
def _provision_flow_checkpoint_summary(checkpoints: List[Dict[str, Any]]) -> Dict[str, Any]:
    filtered = [dict(item) for item in checkpoints if isinstance(item, dict)]
    completed = sum(1 for item in filtered if str(item.get("status", "")).strip().lower() == "completed")
    failed = sum(1 for item in filtered if str(item.get("status", "")).strip().lower() == "failed")
    in_progress = sum(1 for item in filtered if str(item.get("status", "")).strip().lower() == "in_progress")
    ...
    return {
        ...
        "final_stage": str(filtered[-1].get("stage", "")).strip().lower() if filtered else "",
        "final_status": str(filtered[-1].get("status", "")).strip().lower() if filtered else "",
    }
...
def _compensation_actions_from_created(created: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for port in sorted({int(item) for item in created.get("ports", set()) if str(item).isdigit()}):
        actions.append(
            {
                "resource_type": "port",
...
def _compensation_summary(events: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    return {
        "event_count": len(filtered),
        "applied_count": sum(1 for item in filtered if bool(item.get("applied", False))),
        "rollback_action_count": sum(
            len(item.get("actions", []))
            for item in filtered
            if isinstance(item.get("actions"), list)
        ),
        "evidence_preserved_count": sum(1 for item in filtered if bool(item.get("evidence_preserved", False))),
    }


def _deterministic_decision_reasons(*, blocked: bool, issues: List[Dict[str, Any]]) -> List[Dict[str, str]]:
    error_codes = sorted(
        {
...
def _validate_security_reference_contracts(
    *,
    ...
    available_connection_profiles = _normalize_reference_catalog(connection_profile_registry)
    ...
    for idx, host in enumerate(normalized_hosts):
        ...
        if not connection_profile_ref:
            ...
            continue
        if available_connection_profiles and connection_profile_ref.lower() not in available_connection_profiles:
            issues.append(
                ProvisionIssue(
                    level="error",
    ...
    for org_id, profile in sorted(org_crypto_profiles.items(), key=lambda item: str(item[0])):
...
def _normalize_host_state(host_state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    normalized["provider"] = str(normalized.get("provider", "external-vm")).strip().lower()
            ...
            normalized_components.append(
                {
                    ...
                    "command": sanitize_sensitive_text(str(item.get("command", "")).strip()),
                    ...
                    "healthcheck_command": sanitize_sensitive_text(str(item.get("healthcheck_command", "")).strip()),
                    "service_context": _normalized_service_context(item.get('service_context', {})),
    ...
    normalized["allocated_ports"] = _normalize_port_mapping(normalized.get("allocated_ports", {}))
    ...
    if isinstance(directory_permissions_raw, dict):
        normalized["directory_permissions"] = {
            ...
            for path, permission in sorted(directory_permissions_raw.items(), key=lambda item: str(item[0]))
    ...
    if isinstance(crypto_services_raw, dict):
        ...
        for org_id, service in sorted(crypto_services_raw.items(), key=lambda item: str(item[0])):
    ...
    if isinstance(msp_tls_artifacts_raw, dict):
        for path, artifact in sorted(msp_tls_artifacts_raw.items(), key=lambda item: str(item[0])):
    ...
    normalized["inject_msp_tls_chain_mismatch"] = bool(normalized.get("inject_msp_tls_chain_mismatch", False))
    normalized["inject_msp_tls_key_mismatch"] = bool(normalized.get("inject_msp_tls_key_mismatch", False))
    return normalized
...
def _nodes_by_host_from_plan(execution_plan: Dict[str, Any]) -> Dict[str, List[Dict[str, Any]]]:
...
def _chaincode_runtime_sort_key(runtime_entry: Dict[str, Any]) -> Tuple[str, str, str, str, str, str]:
    return (
...
def _materialize_chaincode_runtime_plan(manifest_report: Dict[str, Any]) -> Dict[str, Any]:
    normalized_components = (
    ...
    scoped_channels = sorted(
        {
            ...
            for item in (scoped_channels_raw if isinstance(scoped_channels_raw, list) else [])
            if str(item).strip()
        }
    )
    ...
    for runtime in runtimes_raw:
        ...
        entry = {
            ...
            "desired_state": str(runtime.get("desired_state", "")).strip().lower() or "required",
            "criticality": str(runtime.get("criticality", "")).strip().lower() or "critical",
            "template": dict(runtime.get("template", {})) if isinstance(runtime.get("template"), dict) else {},
        }
        reasons: List[str] = []
    ...
    filtered_entries = sorted(
        ...
        key=lambda item: (
            *_chaincode_runtime_sort_key(item),
            ",".join(
                sorted({str(reason).strip().lower() for reason in (item.get("ineligible_reasons") or [])})
            ),
        ),
    )
    ...
    runtime_plan_fingerprint = payload_sha256(
        {
            "manifest_fingerprint": str(manifest_report.get("manifest_fingerprint", "")).strip().lower(),
...
def _expected_runtime_name_for_entry(
    *,
    ...
    peer_name = str(peer_name_by_component_id.get(str(target_peer).strip().lower(), "")).strip().lower()
    if not peer_name:
        return ""
...
def _materialize_chaincode_runtime_template_contract(
    *,
    ...
    environment_profile_ref = str(manifest_report.get("environment_profile_ref", "")).strip().lower()
    environment_profile_aliases = _environment_profile_aliases(environment_profile_ref)
    catalog_entry = dict(CCTOOLS_RUNTIME_TEMPLATE_CATALOG.get(DEFAULT_CCTOOLS_RUNTIME_TEMPLATE_ID, {}))
    entries_raw = (
    ...
    for index, runtime_entry in enumerate(entries_raw):
        ...
        template_id = str(runtime_template.get("template_id", "")).strip() or DEFAULT_CCTOOLS_RUNTIME_TEMPLATE_ID
        template_version = str(runtime_template.get("template_version", "")).strip() or str(
            catalog_entry.get("template_version", "")
        ).strip()
        engine = str(runtime_template.get("engine", "")).strip().lower() or CCTOOLS_RUNTIME_TEMPLATE_ENGINE
        contract_version = str(runtime_template.get("contract_version", "")).strip() or CCTOOLS_RUNTIME_TEMPLATE_CONTRACT_VERSION
        language_runtime = str(runtime_template.get("language_runtime", "")).strip().lower() or str(
            catalog_entry.get("language_runtime", "")
        ).strip().lower()
        ...
        declared_mounts = _normalize_runtime_template_mounts(runtime_template.get("mounts", []))
        default_mounts = _normalize_runtime_template_mounts(catalog_entry.get("mounts", []))
        mounts = declared_mounts or default_mounts
        mounts_required = bool(
            runtime_template.get("mounts_required", catalog_entry.get("mounts_required", False))
        )
        ...
        compatible_profiles = sorted(
            {
                str(item).strip().lower()
                for item in (compatible_profiles_raw if isinstance(compatible_profiles_raw, list) else [])
                if str(item).strip()
            }
        )
        ...
        bootstrap_parameters = {
            ...
            "package_id_hash": str(runtime_entry.get("package_id_hash", "")).strip().lower(),
        ...
        for env_key, env_value in sorted(bootstrap_parameters["env"].items(), key=lambda item: str(item[0])):
        ...
        if language_runtime_declared and not str(runtime_template.get("language_runtime", "")).strip():
            issues.append(
                {
                    "level": "error",
                    "code": "runtime_template_missing_required_parameter",
        ...
        if not isinstance(bootstrap_parameters["env"], dict) or not bootstrap_parameters["env"]:
            issues.append(
                {
                    "level": "error",
                    "code": "runtime_template_missing_required_parameter",
        ...
        if not isinstance(bootstrap_parameters["ports"], list) or not bootstrap_parameters["ports"]:
            issues.append(
                {
                    "level": "error",
                    "code": "runtime_template_missing_required_parameter",
        ...
        resolved_entries.append(
            {
                ...
                "target_peer": bootstrap_parameters["target_peer"],
                "runtime_name": runtime_name,
                "desired_state": str(runtime_entry.get("desired_state", "")).strip().lower() or "required",
                "criticality": str(runtime_entry.get("criticality", "")).strip().lower() or "critical",
    ...
    for entry in resolved_entries:
        ...
        binding = "|".join(
            [
                ...
                str((entry.get("bootstrap_parameters") or {}).get("package_id_hash", "")).strip().lower(),
            ]
        )
        runtime_name_bindings.setdefault(runtime_name_key, set()).add(binding)
    for runtime_name_key, bindings in sorted(runtime_name_bindings.items(), key=lambda item: item[0]):
        if len(bindings) <= 1:
            continue
        issues.append(
            {
...
def _runtime_env_file_content(env: Dict[str, str]) -> str:
    lines = [f"{str(key)}={str(value)}" for key, value in sorted(env.items(), key=lambda item: str(item[0]))]
    return "\n".join(lines) + ("\n" if lines else "")
...
def _is_safe_sensitive_literal(value: str) -> bool:
    ...
    if not normalized:
        return True
    lowered = normalized.lower()
    if "***redacted***" in lowered or "<redacted>" in lowered or "[redacted]" in lowered:
        return True
...
def _collect_sensitive_literal_findings(payload: Any, *, path: str) -> List[Dict[str, str]]:
...
def _materialize_incremental_security_report(
    *,
    ...
    incremental_new_entries = [
        ...
        if isinstance(item, dict)
        and str(item.get("operation_type", "")).strip().lower().startswith("incremental_")
    ...
    for host_id in target_host_ids:
        ...
        connection_profile_ref = str(host_payload.get("connection_profile_ref", "")).strip()
        if not connection_profile_ref:
    ...
    if manifest_org_id and isinstance(org_crypto_profiles.get(manifest_org_id), dict):
        ...
        for org_key, candidate in sorted(org_crypto_profiles.items(), key=lambda item: str(item[0])):
        ...
        if not profile and len(org_crypto_profiles) == 1:
            profile_org_key = str(next(iter(org_crypto_profiles.keys()))).strip().lower()
    ...
    incremental_entry_keys = {
        (str(item.get("host_id", "")).strip(), str(item.get("component_id", "")).strip())
        for item in incremental_new_entries
    }
    incremental_ssh_units = [
        ...
        and (
            (str(unit.get("host_id", "")).strip(), str(unit.get("component_id", "")).strip())
...
def _incremental_origin_inventory_metadata(
    *,
    ...
    try:
        ...
        base_generation = 0
    topology_generation = base_generation + 1 if incremental_entries else base_generation
    ...
    *,
    ...
    for payload_name, payload in required_payloads.items():
        ...
        for field_name in (
            ...
            expected_value = str(expected_correlation.get(field_name, "")).strip().lower()
...
def _materialize_chaincode_runtime_bundle(
    *,
    ...
    for entry in entries:
        ...
        if not runtime_name:
            continue
        bootstrap = entry.get("bootstrap_parameters") if isinstance(entry.get("bootstrap_parameters"), dict) else {}
        template_contract = entry.get("template") if isinstance(entry.get("template"), dict) else {}
        runtime_dir = f"runtime-bundle/{run.run_id}/{_runtime_bundle_safe_segment(runtime_name)}"
        ...
        runtime_image = str(env.get("CHAINCODE_RUNTIME_IMAGE", "")).strip() or DEFAULT_CHAINCODE_RUNTIME_IMAGE
        runtime_command = str(env.get("CHAINCODE_RUNTIME_COMMAND", "")).strip() or DEFAULT_CHAINCODE_RUNTIME_COMMAND
        runtime_command_sanitized = _sanitize_runtime_command_for_publication(runtime_command)
        runtime_command_digest = hashlib.sha256(runtime_command.encode("utf-8")).hexdigest()
        ...
        runtime_spec = {
            ...
            "template": {
                ...
                "template_version": str(template_contract.get("template_version", "")).strip(),
                "engine": str(template_contract.get("engine", "")).strip().lower(),
                "contract_version": str(template_contract.get("contract_version", "")).strip(),
            },
            "runtime": {
                "language_runtime": language_runtime,
                "image": runtime_image,
        ...
        files = [
            {
                ...
                "kind": "runtime_spec",
                "content": json.dumps(runtime_spec, ensure_ascii=False, indent=2, sort_keys=True).encode("utf-8"),
            },
            {
        ...
        rendered_entries.append(
            {
                ...
                "version": str(entry.get("version", "")).strip().lower(),
                "target_peer": str(entry.get("target_peer", "")).strip().lower(),
                "files": sorted(file_rows, key=lambda item: str(item.get("artifact_name", ""))),
...
def _api_runtime_peer_names_by_org(manifest_report: Dict[str, Any]) -> Dict[str, List[str]]:
    components = (
        manifest_report.get("normalized_components")
        if isinstance(manifest_report.get("normalized_components"), list)
        else []
    ...
    for comp in components:
        ...
        name = str(comp.get("name", "")).strip() or str(comp.get("component_id", "")).strip()
        if org_id and name:
...
def _api_runtime_effective_org_id(*, service_context: Dict[str, Any], manifest_org_id: str) -> str:
...
def _materialize_api_runtime_bootstrap_for_entry(
    *,
    ...
    org_id = _api_runtime_effective_org_id(service_context=service_context, manifest_org_id=manifest_org_id)

    channel_ids = (
        [str(c).strip().lower() for c in (service_context.get("channel_ids") or []) if str(c).strip()]
    ...
    chaincode_ids = (
        [str(c).strip().lower() for c in (service_context.get("chaincode_ids") or []) if str(c).strip()]
        if isinstance(service_context, dict)
        else []
    )
    ...
    if orderer_name:
        connection_profile["orderers"][orderer_name] = {"url": f"grpc://{orderer_name}:7050"}

    for ch in channel_ids:
        connection_profile["channels"][ch] = {"peers": {peer_name: {}}} if peer_name else {"peers": {}}
    ...
    identity_entry: Dict[str, Any] = {
        ...
        "discoveryAsLocalhost": bool(service_context.get("discoveryAsLocalhost", False)),
    }
    identities = {
    ...
    fingerprint = payload_sha256(
        {"org_id": org_id, "peer_name": peer_name, "orderer_name": orderer_name, "channels": channel_ids, "chaincodes": chaincode_ids}
    )
    return {
        "org_id": org_id,
        "peer_name": peer_name,
...
def _api_runtime_bootstrap_host_prep_command(*, template: Dict[str, Any], bootstrap: Dict[str, Any]) -> str:
    storage_mount = template.get("storage_mount", {}) if isinstance(template.get("storage_mount"), dict) else {}
        ...
        + _shell_single_quote(
            "\n".join(
                [
                    ...
                    "if [ ! -s " + shlex.quote(cert_path) + " ] || [ ! -s " + shlex.quote(key_path) + " ]; then",
                    "  if command -v openssl >/dev/null 2>&1; then",
                    "    openssl req -x509 -newkey rsa:2048 -nodes "
                    + "-keyout " + shlex.quote(key_path) + " -out " + shlex.quote(cert_path)
                    + " -days 365 -subj " + shlex.quote("/CN=" + org_id) + " >/dev/null 2>&1 || true",
                    "  else",
                    ...
                    "fi",
                        ...
                        "echo 'wallet_probe_test' > " + shlex.quote(f"{host_dir}/wallet/probe.txt") + " || true",
                    ...
                    "JSON",
                    "chmod 600 "
                    + shlex.quote(conn_path) + " " + shlex.quote(ids_path) + " "
                    + shlex.quote(cert_path) + " " + shlex.quote(key_path)
                    + " || true",
                ]
            )
        )
...
def _runtime_bootstrap_command_from_entry(entry: Dict[str, Any]) -> str:
    ...
    ports = _sorted_unique_positive_ints(bootstrap.get("ports", []))
    mounts = _normalize_runtime_template_mounts(bootstrap.get("mounts", []))
    runtime_image = str(env.get("CHAINCODE_RUNTIME_IMAGE", "")).strip() or "hyperledger/fabric-ccenv:2.5"
    runtime_command = str(env.get("CHAINCODE_RUNTIME_COMMAND", "")).strip() or "chaincode-entrypoint"
    ...
    for mount in mounts:
        ...
        if not host_path or not container_path:
            continue
        suffix = ":ro" if read_only else ""
        run_parts.append(f"-v {_shell_single_quote(f'{host_path}:{container_path}{suffix}') }")
    run_parts.append(_shell_single_quote(runtime_image))
    run_parts.append(runtime_command)

    cleanup_cmd = f"docker rm -f {_shell_single_quote(runtime_name)} >/dev/null 2>&1 || true"
...
def _materialize_chaincode_runtime_bootstrap_plan(
    *,
    ...
    for entry in bundle_entries:
        ...
        if not runtime_name:
            continue
        template_entry = template_entries_by_runtime.get(runtime_name.strip().lower(), {})
        bootstrap_parameters = (
            dict(template_entry.get("bootstrap_parameters", {}))
            if isinstance(template_entry.get("bootstrap_parameters"), dict)
            else {}
        )
        endpoint_ports = _sorted_unique_positive_ints(bootstrap_parameters.get("ports", []))
        host_id = str(peer_host_index.get(target_peer, "")).strip()
        if not host_id:
            unresolved_entries += 1
        entry_fingerprint = str(entry.get("runtime_bundle_entry_fingerprint", "")).strip().lower()
        ...
        plan_entries.append(
            {
                ...
                "desired_state": str(template_entry.get("desired_state", "")).strip().lower() or "required",
                "criticality": str(template_entry.get("criticality", "")).strip().lower() or "critical",
            ...
            "entries": [
                {
                    ...
                    "command_digest": str(item.get("command_digest", "")).strip().lower(),
...
def _runtime_template_contract_public_projection(contract: Dict[str, Any]) -> Dict[str, Any]:
    projected = dict(contract)
    entries = (
        contract.get("entries")
        if isinstance(contract.get("entries"), list)
        else []
    )
...
def _runtime_bootstrap_plan_public_projection(runtime_bootstrap_plan: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for entry in entries:
        ...
        if command:
            ...
            item["command_digest"] = str(item.get("command_digest", "")).strip().lower() or hashlib.sha256(
...
def _compact_runtime_verify_rows(rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for row in rows:
        if not isinstance(row, dict):
            continue
        smoke = row.get("smoke_check") if isinstance(row.get("smoke_check"), dict) else {}
        compacted.append(
            {
                "host_id": str(row.get("host_id", "")).strip(),
                ...
                "verify_error_classification": str(row.get("verify_error_classification", "")).strip().lower(),
                "smoke_status": str(smoke.get("status", "")).strip().lower(),
                "smoke_error_classification": str(smoke.get("error_classification", "")).strip().lower(),
...
def _runtime_reconcile_expected_entries(
    *,
    ...
    for entry in (
        ...
        expected_command = str(env.get("CHAINCODE_RUNTIME_COMMAND", "")).strip() or DEFAULT_CHAINCODE_RUNTIME_COMMAND
        expected_rows.append(
            {
                ...
                "desired_state": str(entry.get("desired_state", "")).strip().lower() or "required",
                "criticality": str(entry.get("criticality", "")).strip().lower() or "critical",
                "expected_ports": _sorted_unique_positive_ints(bootstrap.get("ports", [])),
                "expected_image": str(env.get("CHAINCODE_RUNTIME_IMAGE", "")).strip() or DEFAULT_CHAINCODE_RUNTIME_IMAGE,
                "expected_command": _sanitize_runtime_command_for_publication(expected_command),
                "expected_command_digest": hashlib.sha256(expected_command.encode("utf-8")).hexdigest(),
            }
        )
    return sorted(
        expected_rows,
        ...
        observed_payload.update(
            {
                ...
                "health": str(verify.get("health", "unknown")).strip().lower() or "unknown",
...
def _runtime_reconcile_justification(
    *,
    ...
    normalized_reasons = sorted({str(item).strip().lower() for item in reasons if str(item).strip()})
    ...
    if normalized_divergence == "drifted":
        return (
            f"runtime_drift_detected:{reason_suffix}" if reason_suffix else "runtime_drift_detected",
    ...
    if normalized_divergence == "stale":
        return (
            f"runtime_stale_detected:{reason_suffix}" if reason_suffix else "runtime_stale_detected",
...
def _materialize_chaincode_runtime_reconcile_report(
    *,
    ...
    for expected in expected_entries:
        ...
        desired_state = str(expected.get("desired_state", "")).strip().lower() or "required"
        ...
        if not observed:
            ...
            expected_ports = _sorted_unique_positive_ints(expected.get("expected_ports", []))
        ...
        rows.append(
            {
                ...
                "decision": decision,
                "converged": converged,
                "reasons": sorted({str(item).strip().lower() for item in reasons if str(item).strip()}),
                ...
                "expected": {
                    ...
                    "ports": _sorted_unique_positive_ints(expected.get("expected_ports", [])),
                },
                "observed": observed,
            }
        )
    ...
    for key in sorted(observed_index.keys(), key=lambda item: (str(item[0]), str(item[1]))):
    ...
    required_non_converged = [
        {
            ...
            "justification_code": str(item.get("justification_code", "")).strip().lower(),
            "org_id": str(item.get("org_id", "")).strip().lower(),
        }
    ...
    runtime_reconcile_fingerprint = payload_sha256(
        {
            ...
            "runtime_bootstrap_plan_fingerprint": str(
                chaincode_runtime_bootstrap_plan.get("runtime_bootstrap_plan_fingerprint", "")
            ).strip().lower(),
...
def _incremental_component_index_from_runtime_state(
    ...
    for host_id in sorted(runtime_state.keys()):
        host_payload = runtime_state.get(host_id, {}) if isinstance(runtime_state.get(host_id), dict) else {}
        for component in (host_payload.get("components") or []):
            ...
            if not component_name and not component_id:
                continue
            payload = {
                "host_id": str(component.get("host_id", "")).strip() or str(host_id).strip(),
                "component_id": component_id,
                "name": component_name,
                "component_type": str(component.get("component_type", "")).strip().lower(),
                "image": str(component.get("image", "")).strip(),
                "ports": _sorted_unique_positive_ints(component.get("ports", [])),
                "status": str(component.get("status", "")).strip().lower(),
                "running": str(component.get("status", "")).strip().lower() == "running",
            }
            if component_id:
...
def _incremental_out_of_scope_justification_index(topology_change_intent: Dict[str, Any]) -> Dict[Tuple[str, str], Dict[str, str]]:
    justification_root = (
    ...
    if not isinstance(justification_items, list):
        justification_items = topology_change_intent.get("out_of_scope_change_justifications")
    if not isinstance(justification_items, list):
        return {}
...
def _materialize_incremental_reconcile_report(
    *,
    ...
    baseline_index = _incremental_component_index_from_observed_state(observed_state_baseline)
    final_index = _incremental_component_index_from_runtime_state(normalized_runtime_state)
    justification_index = _incremental_out_of_scope_justification_index(topology_change_intent)
    ...
    for item in entries:
        ...
        expected = {
            ...
            "component_type": str(item.get("component_type", "")).strip().lower(),
            "desired_state": str(item.get("desired_state", "required")).strip().lower() or "required",
            "criticality": str(item.get("criticality", "critical")).strip().lower() or "critical",
            "image": str(item.get("image", "")).strip(),
            "ports": _sorted_unique_positive_ints(item.get("ports", [])),
    ...
    for expected in sorted(
        ...
        final_observed = final_index.get((host_id, component_id.lower())) if component_id else None
        if final_observed is None and component_name:
            final_observed = final_index.get((host_id, component_name.lower()))

        baseline_observed = baseline_index.get((host_id, component_id.lower())) if component_id else None
        ...
        if final_observed is None:
            ...
            if "running" not in final_observed:
                running = str(final_observed.get("status", "")).strip().lower() == "running"
            image_drift = bool(
            ...
            observed_ports = _sorted_unique_positive_ints(final_observed.get("ports", []))
            port_drift = bool(expected_ports and observed_ports and expected_ports != observed_ports)
        ...
        row = {
            ...
            "observed": dict(final_observed) if isinstance(final_observed, dict) else {},
        }
        rows.append(row)
    ...
    out_of_scope_keys = sorted(set(baseline_canonical.keys()) | set(final_canonical.keys()))
    for key in out_of_scope_keys:
        ...
        component_type = str((after or before or {}).get("component_type", "")).strip().lower()
        ...
        if before is None and after is not None:
            ...
            if before_status == "running" and after_status and after_status != "running":
                changed = True
                decision = "restart"
        ...
        rows.append(
            {
                "scope": "out_of_scope",
                "host_id": str(key[0]).strip(),
                "component_id": str((after or before or {}).get("component_id", "")).strip(),
...
def _materialize_incremental_operational_continuity_report(
    *,
    ...
    for key in sorted(baseline_canonical.keys(), key=lambda item: (str(item[0]), str(item[1]))):
        before = baseline_canonical.get(key, {})
        ...
        payload = {
            ...
            "observed_impact": "preserved" if (after is not None and after_running) else "availability_regression",
        }
    ...
    summary = {
        "pre_existing_mandatory_healthy_count": len(preserved_components) + len(availability_regressions),
...
def _materialize_chaincode_runtime_inventory_rows(
    *,
    ...
    for item in runtime_reconcile_rows:
        ...
        if not runtime_name:
            continue
        image = str(observed.get("image", "")).strip() or str(expected.get("image", "")).strip()
        command = str(expected.get("command", "")).strip() or DEFAULT_CHAINCODE_RUNTIME_COMMAND
        ports = _sorted_unique_positive_ints(observed.get("ports", [])) or _sorted_unique_positive_ints(
            expected.get("ports", [])
        )
...
def _ssh_unit_with_error_classification_alias(unit: Dict[str, Any]) -> Dict[str, Any]:
    ...
    normalized["attempts"] = [
        {
            **dict(item),
            "error_classification": str((item or {}).get("classification", "")).strip().lower(),
        }
...
def _all_channel_locks(blueprint_validation: BlueprintValidationResult) -> List[Tuple[str, str]]:
...
def _host_runtime_inventory(host_ref: str, host_state: Dict[str, Any]) -> Dict[str, Any]:
    nodes = []
    ...
    for org_id in sorted(host_state.get("crypto_services", {}).keys()):
        ...
        ca_payload = dict(service.get("ca", {})) if isinstance(service.get("ca", {}), dict) else {}
        tls_ca_payload = dict(service.get("tls_ca", {})) if isinstance(service.get("tls_ca", {}), dict) else {}
        ...
        crypto_services.append(
            {
                ...
                "issued_identities": sorted(
                    [item for item in (service.get("issued_identities") or []) if isinstance(item, dict)],
                    key=lambda item: (
                        str(item.get("identity_id", "")),
                        str(item.get("status", "")),
                        int(item.get("issuance_version", 0)) if str(item.get("issuance_version", "")).isdigit() else 0,
                ...
                "issuance_events": sorted(
                    [item for item in (service.get("issuance_events") or []) if isinstance(item, dict)],
                    key=lambda item: (
                        str(item.get("identity_id", "")),
                        str(item.get("event", "")),
                        int(item.get("issuance_version", 0)) if str(item.get("issuance_version", "")).isdigit() else 0,
    ...
    component_postchecks = sorted(
        [dict(item) for item in (host_state.get("component_postchecks") or []) if isinstance(item, dict)],
        key=lambda item: (
            str(item.get("component_type", "")),
            str(item.get("component_id", "")),
            str(item.get("name", "")),
    ...
    active_fabric_base = [
        ...
        if _is_fabric_base_component_type(str(item.get("component_type", "")).strip().lower())
        and str(item.get("status", "")).strip().lower() == "running"
    ]
    active_peers = [
        item for item in active_fabric_base if str(item.get("component_type", "")).strip().lower() == "peer"
    ]
    active_couches = [
        item for item in active_fabric_base if str(item.get("component_type", "")).strip().lower() == "couch"
    ]
    pairing_ok = len(active_couches) >= len(active_peers)
    active_api_components = [
        item
        for item in components
        if _is_api_runtime_component_type(str(item.get("component_type", "")).strip().lower())
        and str(item.get("status", "")).strip().lower() == "running"
    ]
    api_endpoint_operational_count = sum(
        1 for item in active_api_components if bool(item.get("endpoint_operational", False))
    )
    return {
        "host_ref": host_ref,
        ...
        "allocated_ports": sorted(int(port) for port in host_state.get("allocated_ports", {}).keys()),
        ...
        "fabric_base_summary": {
            ...
            "active_ca_count": len(
                [
                    item for item in active_fabric_base if str(item.get("component_type", "")).strip().lower() == "ca"
                ]
            ),
        ...
        "api_runtime_summary": {
            ...
            "active_api_gateway_count": len(
                [
                    ...
                    if str(item.get("component_type", "")).strip().lower() == "api_gateway"
                ]
            ),
            "active_network_api_count": len(
                [
                    item
                    for item in active_api_components
                    if str(item.get("component_type", "")).strip().lower() == "network_api"
                ]
            ),
        ...
        "msp_tls_artifacts": [
            ...
            for artifact_path in sorted((host_state.get("msp_tls_artifacts") or {}).keys())
            if isinstance(host_state.get("msp_tls_artifacts", {}).get(artifact_path), dict)
        ],
        "msp_tls_manifests": sorted(
            [item for item in (host_state.get("msp_tls_manifests") or []) if isinstance(item, dict)],
            key=lambda item: (
                str(item.get("org_id", "")),
                str(item.get("node_id", "")),
                str(item.get("node_type", "")),
...
def _materialize_msp_tls_artifacts_for_node(
    *,
    ...
    tls_ca_info = service.get("tls_ca", {}) if isinstance(service.get("tls_ca"), dict) else {}

    root_subject = str(((ca_info.get("root_certificate") or {}).get("subject")) or "").strip()
    intermediate_subject = str(((ca_info.get("intermediate_certificate") or {}).get("subject")) or "").strip()
    tls_root_subject = str(((tls_ca_info.get("root_certificate") or {}).get("subject")) or "").strip()
    tls_intermediate_subject = str(((tls_ca_info.get("intermediate_certificate") or {}).get("subject")) or "").strip()
    ...
    private_key_id = public_key_id if not inject_key_mismatch else payload_sha256({"mismatch": public_key_id})[:24]
    effective_intermediate_subject = (
        f"CN=InvalidIntermediate.{domain}" if inject_chain_mismatch else intermediate_subject
    )
    ...
    artifact_payloads = [
        ...
        },
        {
            ...
            "content": {
                ...
                "subjects": [subject for subject in [tls_root_subject, tls_intermediate_subject] if subject],
            },
        },
    ]
    ...
    key_cert_match = bool(public_key_id and private_key_id and public_key_id == private_key_id)
    trust_bundle_hashes = [
    ...
    manifest_payload = {
        ...
        "msp_layout": {
            "directories": [f"{msp_base_dir}/{directory}" for directory in MSP_LAYOUT_DIRS],
...
def _build_org_host_targets(
    ...
    for org in sorted(blueprint_validation.normalized_orgs, key=lambda item: str(item.get("org_id", ""))):
...
def _build_org_domain_index(blueprint_validation: BlueprintValidationResult) -> Dict[str, str]:
    return {
        str(org.get("org_id", "")).strip().lower(): str(org.get("domain", "")).strip().lower()
        for org in blueprint_validation.normalized_orgs
        if str(org.get("org_id", "")).strip()
    }


def _build_org_identity_contracts(blueprint_validation: BlueprintValidationResult) -> Dict[str, Dict[str, Any]]:
    ...
    for org in blueprint_validation.normalized_orgs:
        ...
        contracts[org_id] = {
            ...
            "roles": sorted(set(str(item).strip().lower() for item in (org.get("roles") or []) if str(item).strip())),
            ...
            "node_identity_allowed_roles": sorted(
                set(
                    str(item).strip().lower()
                    for item in ((identity.get("node_identity_policy") or {}).get("allowed_roles") or [])
                    if str(item).strip()
                )
            ),
            "admin_identity_allowed_roles": sorted(
                set(
                    str(item).strip().lower()
                    for item in ((identity.get("admin_identity_policy") or {}).get("allowed_roles") or [])
                    if str(item).strip()
                )
            ),
        }
...
def _build_identity_targets(
    *,
    ...
    for org_id, contract in sorted(org_contracts.items(), key=lambda item: item[0]):
        ...
        for node in sorted(nodes_by_org.get(org_id, []), key=lambda item: str((item or {}).get("node_id", ""))):
            node_type = str((node or {}).get("node_type", "")).strip().lower()
...
def _identity_naming_mismatch(target: Dict[str, Any], material: Dict[str, Any]) -> Optional[str]:
    org_id = str(target.get("org_id", "")).strip().lower()
    msp_id = str(target.get("msp_id", "")).strip()
    domain = str(target.get("domain", "")).strip().lower()
    node_id = str(target.get("node_id", "")).strip().lower()
    ...
    san_dns = sorted(set(str(item).strip().lower() for item in (material.get("san_dns") or []) if str(item).strip()))
...
def _should_reenroll_identity(existing_active: Dict[str, Any], desired: Dict[str, Any], key_size: int) -> bool:
    if str(existing_active.get("subject", "")) != str(desired.get("subject", "")):
        return True
    existing_san = sorted(set(str(item).strip() for item in (existing_active.get("san_dns") or []) if str(item).strip()))
    desired_san = sorted(set(str(item).strip() for item in (desired.get("san_dns") or []) if str(item).strip()))
    ...
    if int(existing_active.get("key_size", 0)) != int(key_size):
        return True
    if str(existing_active.get("ou", "")).strip().lower() != str(desired.get("ou", "")).strip().lower():
        return True
    return False
...
def _plan_entries_for_host(
    ...
    filtered = [
        ...
        if isinstance(item, dict)
        and str(item.get("host_id", "")).strip() == host_key
    ]
...
def _is_managed_runtime_component_type(component_type: str) -> bool:
    ...
    return normalized in FABRIC_BASE_COMPONENT_TYPES or normalized in API_RUNTIME_COMPONENT_TYPES

def _resolve_runtime_template_for_entry(entry: Dict[str, Any]) -> Dict[str, Any]:

    component_type = _canonical_component_type(str(entry.get("component_type", "")).strip())
    defaults = RUNTIME_TEMPLATE_DEFAULTS.get(component_type, {})
    component_id = str(entry.get("component_id", "")).strip()
    component_name = str(entry.get("name", "")).strip() or component_id or "component-unknown"
    image = str(entry.get("image", "")).strip() or str(defaults.get("image", "")).strip() or "<resolved-by-runtime>"
    ports = _sorted_unique_positive_ints(entry.get("ports", [])) or _sorted_unique_positive_ints(defaults.get("ports", []))
    env_profile = str(entry.get("env_profile", "")).strip().lower() or "default"
    storage_profile = str(entry.get("storage_profile", "")).strip() or f"{component_type}-persistent"
    resources = _normalize_resource_contract(entry.get("resources", {}))
    default_env = defaults.get("default_env", {}) if isinstance(defaults.get("default_env"), dict) else {}
    service_context = _normalized_service_context(entry.get('service_context', {}))
    env = {
        **{str(key): str(value) for key, value in sorted(default_env.items(), key=lambda item: str(item[0]))},
    ...
    runtime_command = str(entry.get("runtime_command", "")).strip() or str(defaults.get("runtime_command", "")).strip()
    ...
    healthcheck_command = healthcheck_template.format(name=component_name) if healthcheck_template else ""
    return {
        "component_id": component_id,
        "component_type": component_type,
        "name": component_name,
...
def _docker_create_command_for_fabric_template(template: Dict[str, Any]) -> str:
    ...
    args: List[str] = ["docker", "container", "create", "--name", name, "--restart", "unless-stopped"]
    args.extend(["--network", DOCKER_RUNTIME_NETWORK_NAME])
    args.extend(["--label", f"cognus.component_type={component_type}"])
    args.extend(["--label", f"cognus.env_profile={str(template.get('env_profile', '')).strip().lower()}"])
...
def _ssh_command_for_plan_entry(entry: Dict[str, Any]) -> str:
    ...
    if _is_managed_runtime_component_type(component_type):
        template = _resolve_runtime_template_for_entry({**entry, "component_type": component_type})
        create_command = _docker_create_command_for_fabric_template(template)

        bootstrap = entry.get("api_runtime_bootstrap") if isinstance(entry.get("api_runtime_bootstrap"), dict) else {}
        bootstrap_cmd = ""
        if bootstrap and _is_api_runtime_component_type(component_type):
            bootstrap_cmd = _api_runtime_bootstrap_host_prep_command(template=template, bootstrap=bootstrap)
        ...
        if action == "update":
            return _with_bootstrap(f"docker container rm -f {target} || true && {create_command}")
        if action == "start":
...
def _postcheck_fabric_component(
    *,
    ...
    expected_status = "running" if action in {"create", "update", "start", "noop"} else "unknown"
    ...
    observed_health = str(component_payload.get("health_status", "")).strip().lower() or "unknown"
    ...
    checks = {
        ...
        "process_active": (observed_status == "running") if expected_status == "running" else True,
        "ports_expected": observed_ports == expected_ports,
        "image_expected": (
            str(component_payload.get("image", "")).strip() == str(template.get("image", "")).strip()
        ),
    ...
    return {
        ...
        "expected": {
            ...
            "endpoint": endpoint if _is_api_runtime_component_type(component_type) else "",
        },
        "observed": {
...
def _api_gateway_connection_profile_payload(
    *,
    ...
    channels = sorted({str(c).strip().lower() for c in channel_ids if str(c).strip()}) or ["default"]

    peers: Dict[str, Any] = {}
    organizations: Dict[str, Any] = {
        normalized_org: {"mspid": normalized_msp, "peers": [peer_container] if peer_container else []}
    }
    ...
    if peer_container:
        peers[peer_container] = {
            ...
            "tlsCACerts": {"path": "/var/cognus/api-runtime/msp/tlscacerts/tlsca-cert.pem"},
        }
...
def _api_gateway_identity_bootstrap_command(
    *,
    ...
    service_context = _normalized_service_context(entry.get('service_context', {}))
    org_id = str(service_context.get("org_id", "")).strip().lower() or str(manifest_org_id).strip().lower() or "default"
    channel_ids = [str(c).strip().lower() for c in (service_context.get("channel_ids") or []) if str(c).strip()]
    if not channel_ids:
        channel_ids = sorted(scoped_channels) or ["default"]

    msp_id = str((org_identity_contracts.get(org_id, {}) or {}).get("msp_id", "")).strip()
    if not msp_id:
        msp_id = f"{org_id.capitalize()}MSP" if org_id and org_id != "default" else "DefaultMSP"
    ...
    for payload in fabric_component_map.values():
        ...
        if str(payload.get("status", "")).strip().lower() == "running":
            peer_container = str(payload.get("name", "")).strip() or str(payload.get("component_id", "")).strip()
            if peer_container:
                break
    ...
    if peer_node_id:
        ...
        tlsca_src = f"/var/cognus/crypto/{org_id}/{peer_node_id}/msp/tlscacerts/tlsca-cert.pem"
    ...
    connection_json = json.dumps(connection_payload, ensure_ascii=False, indent=2, sort_keys=True)
    identities_json = json.dumps(identities_payload, ensure_ascii=False, indent=2, sort_keys=True)
...
def _normalize_lock_resources(resources: Iterable[Tuple[str, str]]) -> List[Tuple[str, str]]:
...
def _lock_scope_keys(resources: Iterable[Tuple[str, str]]) -> List[str]:
    return [f"{resource_type}:{resource_id}" for resource_type, resource_id in resources]
    ...
    *,
    ...
    for host in hosts:
        ...
        if host_id and host_id not in parseable_set:
            ...
            merged_host["observed_components_fingerprint"] = payload_sha256(fallback_components)
        else:
...
def run_provision_stage(
    *,
    ...
    if not crypto_preconditions or not bool(crypto_preconditions.get("valid", False)):
        raise ValueError("provision exige pré-condições criptográficas válidas do prepare.")
    ...
    incremental_replay_fingerprint = payload_sha256(
        {
            ...
            "topology_change_intent": topology_change_intent if isinstance(topology_change_intent, dict) else {},
            "topology_change_intent_fingerprint": (
                str(a2_4_incremental_gate_result.topology_change_intent_fingerprint).strip().lower()
                if a2_4_incremental_gate_result is not None
                else ""
            ),
            "incremental_allocation_fingerprint": (
                str(a2_4_incremental_gate_result.incremental_allocation_fingerprint).strip().lower()
                if a2_4_incremental_gate_result is not None
                else ""
            ),
            "incremental_placement_fingerprint": (
                str(a2_4_incremental_gate_result.incremental_placement_fingerprint).strip().lower()
                if a2_4_incremental_gate_result is not None
                else ""
            ),
        }
    ...
    chaincode_runtime_template_contract = _materialize_chaincode_runtime_template_contract(
    ...
    for contract_issue in (
        ...
        issues.append(
            ProvisionIssue(
                level=str(contract_issue.get("level", "error")).strip().lower() or "error",
                code=str(contract_issue.get("code", "runtime_template_invalid")).strip().lower(),
                path=str(contract_issue.get("path", "chaincode_runtime_template_contract")).strip()
                or "chaincode_runtime_template_contract",
                message=str(contract_issue.get("message", "Template runtime inválido para bootstrap cc-tools.")).strip(),
    ...
    _all_api_peers = sorted({p for peers in api_peer_names_by_org.values() for p in (peers or [])}, key=lambda v: v.lower())
    ...
    org_crypto_profiles = {
        ...
        for profile in (org_crypto_profiles_raw if isinstance(org_crypto_profiles_raw, list) else [])
    ...
    if state_store is not None:
        flow_checkpoint_path = state_store.stage_artifacts_dir(run.run_id, "provision") / "provision-flow-checkpoints.json"
        compensation_path = state_store.stage_artifacts_dir(run.run_id, "provision") / "provision-compensation-report.json"
        wp_a22_provision_plan_path = state_store.stage_artifacts_dir(run.run_id, "provision") / "provision-plan.json"
        wp_a22_reconcile_report_path = state_store.stage_artifacts_dir(run.run_id, "provision") / "reconcile-report.json"
        wp_a22_inventory_final_path = state_store.stage_artifacts_dir(run.run_id, "provision") / "inventory-final.json"
        wp_a22_stage_reports_path = state_store.stage_artifacts_dir(run.run_id, "provision") / "stage-reports.json"
        wp_a22_verify_report_path = state_store.stage_artifacts_dir(run.run_id, "provision") / "verify-report.json"
        wp_a22_ssh_log_path = state_store.stage_artifacts_dir(run.run_id, "provision") / "ssh-execution-log.json"
        wp_a22_audit_summary_path = state_store.stage_artifacts_dir(run.run_id, "provision") / "a2-audit-summary.json"
        runtime_plan_path = state_store.stage_artifacts_dir(run.run_id, "provision") / "runtime-plan.json"
        runtime_template_contract_path = state_store.stage_artifacts_dir(run.run_id, "provision") / "runtime-template-contract.json"
        runtime_bundle_manifest_path = state_store.stage_artifacts_dir(run.run_id, "provision") / "runtime-bundle-manifest.json"
        runtime_bootstrap_report_path = state_store.stage_artifacts_dir(run.run_id, "provision") / "runtime-bootstrap-report.json"
        runtime_verify_report_path = state_store.stage_artifacts_dir(run.run_id, "provision") / "runtime-verify-report.json"
        runtime_reconcile_report_path = state_store.stage_artifacts_dir(run.run_id, "provision") / "runtime-reconcile-report.json"
        incremental_plan_path = state_store.stage_artifacts_dir(run.run_id, "provision") / "incremental-execution-plan.json"
        incremental_reconcile_report_path = state_store.stage_artifacts_dir(run.run_id, "provision") / "incremental-reconcile-report.json"
        incremental_security_report_path = state_store.stage_artifacts_dir(run.run_id, "provision") / "incremental-security-report.json"
        if flow_checkpoint_path.exists():
            persisted_flow_checkpoint_report = json.loads(flow_checkpoint_path.read_text(encoding="utf-8"))
        ...
        completed_checkpoint = _stage_completed_checkpoint(state_store=state_store, run=run)
        if completed_checkpoint is not None:
            artifacts: Dict[str, str] = {}
            inventory_path = state_store.stage_artifacts_dir(run.run_id, "provision") / "runtime-inventory.json"
            report_path = state_store.stage_artifacts_dir(run.run_id, "provision") / "provision-report.json"
            plan_path = state_store.stage_artifacts_dir(run.run_id, "provision") / "provision-execution-plan.json"
            observed_state_path = state_store.stage_artifacts_dir(run.run_id, "provision") / "observed-state-baseline.json"
            reconciliation_path = state_store.stage_artifacts_dir(run.run_id, "provision") / "reconciliation-plan.json"
            ssh_execution_path = state_store.stage_artifacts_dir(run.run_id, "provision") / "ssh-execution-report.json"
            lock_report_path = state_store.stage_artifacts_dir(run.run_id, "provision") / "concurrency-lock-report.json"
            flow_checkpoint_path = state_store.stage_artifacts_dir(run.run_id, "provision") / "provision-flow-checkpoints.json"
            compensation_path = state_store.stage_artifacts_dir(run.run_id, "provision") / "provision-compensation-report.json"
            wp_a22_provision_plan_path = state_store.stage_artifacts_dir(run.run_id, "provision") / "provision-plan.json"
            wp_a22_reconcile_report_path = state_store.stage_artifacts_dir(run.run_id, "provision") / "reconcile-report.json"
            ...
            if observed_state_path.exists():
                persisted_observed_baseline = json.loads(observed_state_path.read_text(encoding="utf-8"))
                artifacts["observed_state_baseline"] = str(observed_state_path)
            if reconciliation_path.exists():
                persisted_reconciliation = json.loads(reconciliation_path.read_text(encoding="utf-8"))
                artifacts["reconciliation_plan"] = str(reconciliation_path)
            if ssh_execution_path.exists():
                persisted_ssh_report = json.loads(ssh_execution_path.read_text(encoding="utf-8"))
                artifacts["ssh_execution_report"] = str(ssh_execution_path)
            if lock_report_path.exists():
                persisted_lock_report = json.loads(lock_report_path.read_text(encoding="utf-8"))
                artifacts["concurrency_lock_report"] = str(lock_report_path)
            if flow_checkpoint_path.exists():
                persisted_flow_report = json.loads(flow_checkpoint_path.read_text(encoding="utf-8"))
                artifacts["provision_flow_checkpoints"] = str(flow_checkpoint_path)
            if compensation_path.exists():
                persisted_compensation = json.loads(compensation_path.read_text(encoding="utf-8"))
            ...
            if wp_a22_stage_reports_path.exists():
                persisted_wp_a22_stage_reports = json.loads(wp_a22_stage_reports_path.read_text(encoding="utf-8"))
                artifacts["stage_reports"] = str(wp_a22_stage_reports_path)
            if wp_a22_verify_report_path.exists():
                persisted_wp_a22_verify_report = json.loads(wp_a22_verify_report_path.read_text(encoding="utf-8"))
            ...
            if wp_a22_audit_summary_path.exists():
                persisted_wp_a22_audit_summary = json.loads(wp_a22_audit_summary_path.read_text(encoding="utf-8"))
                artifacts["audit_trail_summary"] = str(wp_a22_audit_summary_path)
            if runtime_plan_path.exists():
                persisted_runtime_plan = json.loads(runtime_plan_path.read_text(encoding="utf-8"))
                artifacts["runtime_plan"] = str(runtime_plan_path)
            if runtime_template_contract_path.exists():
                persisted_runtime_template_contract = json.loads(runtime_template_contract_path.read_text(encoding="utf-8"))
                artifacts["runtime_template_contract"] = str(runtime_template_contract_path)
            if runtime_bundle_manifest_path.exists():
                persisted_runtime_bundle_manifest = json.loads(runtime_bundle_manifest_path.read_text(encoding="utf-8"))
                artifacts["runtime_bundle_manifest"] = str(runtime_bundle_manifest_path)
            if runtime_bootstrap_report_path.exists():
                persisted_runtime_bootstrap_report = json.loads(runtime_bootstrap_report_path.read_text(encoding="utf-8"))
                artifacts["runtime_bootstrap_report"] = str(runtime_bootstrap_report_path)
            if runtime_verify_report_path.exists():
                persisted_runtime_verify_report = json.loads(runtime_verify_report_path.read_text(encoding="utf-8"))
                artifacts["runtime_verify_report"] = str(runtime_verify_report_path)
            if runtime_reconcile_report_path.exists():
                persisted_runtime_reconcile_report = json.loads(runtime_reconcile_report_path.read_text(encoding="utf-8"))
                artifacts["runtime_reconcile_report"] = str(runtime_reconcile_report_path)
            if incremental_reconcile_report_path.exists():
                persisted_incremental_reconcile_report = json.loads(incremental_reconcile_report_path.read_text(encoding="utf-8"))
                artifacts["incremental_reconcile_report"] = str(incremental_reconcile_report_path)
            if incremental_security_report_path.exists():
                persisted_incremental_security_report = json.loads(incremental_security_report_path.read_text(encoding="utf-8"))
                artifacts["incremental_security_report"] = str(incremental_security_report_path)
            ...
            if persisted_plan:
                ...
                persisted_idempotency_semantics = {
                    ...
                    "plan_fingerprint": str(persisted_plan.get("plan_fingerprint", "")).strip().lower(),
                    "mutating_actions": {
                ...
                if "idempotency_semantics" not in provision_report:
                    provision_report["idempotency_semantics"] = persisted_idempotency_semantics
                if "idempotency_semantics" not in runtime_inventory:
                    runtime_inventory["idempotency_semantics"] = persisted_idempotency_semantics
            if persisted_observed_baseline and "observed_state_baseline" not in provision_report:
                provision_report["observed_state_baseline"] = persisted_observed_baseline
            if persisted_observed_baseline and "observed_state_baseline" not in runtime_inventory:
                runtime_inventory["observed_state_baseline"] = persisted_observed_baseline
            ...
            if persisted_lock_report:
                ...
                if inline_lock_report and "concurrency_control" not in runtime_inventory:
            ...
            if persisted_runtime_template_contract:
                if "chaincode_runtime_template_contract" not in provision_report:
                    provision_report["chaincode_runtime_template_contract"] = persisted_runtime_template_contract
                if "chaincode_runtime_template_contract" not in runtime_inventory:
                    runtime_inventory["chaincode_runtime_template_contract"] = persisted_runtime_template_contract
            if persisted_runtime_bundle_manifest:
                if "chaincode_runtime_bundle" not in provision_report:
                    provision_report["chaincode_runtime_bundle"] = persisted_runtime_bundle_manifest
                if "chaincode_runtime_bundle" not in runtime_inventory:
                    runtime_inventory["chaincode_runtime_bundle"] = persisted_runtime_bundle_manifest
            if persisted_runtime_bootstrap_report:
                inline_runtime_bootstrap_report = (
                    dict(persisted_runtime_bootstrap_report.get("inline", {}))
                    if isinstance(persisted_runtime_bootstrap_report.get("inline"), dict)
                    else dict(persisted_runtime_bootstrap_report)
                )
                inline_runtime_bootstrap_report.pop("units_detailed", None)
                if inline_runtime_bootstrap_report and "chaincode_runtime_bootstrap" not in provision_report:
                    provision_report["chaincode_runtime_bootstrap"] = inline_runtime_bootstrap_report
                if inline_runtime_bootstrap_report and "chaincode_runtime_bootstrap" not in runtime_inventory:
                    runtime_inventory["chaincode_runtime_bootstrap"] = inline_runtime_bootstrap_report
            if persisted_runtime_verify_report:
                ...
                if inline_runtime_verify_report and "chaincode_runtime_verify" not in provision_report:
                    provision_report["chaincode_runtime_verify"] = inline_runtime_verify_report
                if inline_runtime_verify_report and "chaincode_runtime_verify" not in runtime_inventory:
                    runtime_inventory["chaincode_runtime_verify"] = inline_runtime_verify_report
            if persisted_runtime_reconcile_report:
                inline_runtime_reconcile_report = (
                    dict(persisted_runtime_reconcile_report.get("inline", {}))
                    if isinstance(persisted_runtime_reconcile_report.get("inline"), dict)
                    else dict(persisted_runtime_reconcile_report)
                )
                inline_runtime_reconcile_report.pop("rows_detailed", None)
                if inline_runtime_reconcile_report and "chaincode_runtime_reconcile" not in provision_report:
                    provision_report["chaincode_runtime_reconcile"] = inline_runtime_reconcile_report
                if inline_runtime_reconcile_report and "chaincode_runtime_reconcile" not in runtime_inventory:
                    runtime_inventory["chaincode_runtime_reconcile"] = inline_runtime_reconcile_report
            if persisted_incremental_reconcile_report:
                if "incremental_reconcile" not in provision_report:
                    provision_report["incremental_reconcile"] = persisted_incremental_reconcile_report
                if "incremental_reconcile" not in runtime_inventory:
                    runtime_inventory["incremental_reconcile"] = persisted_incremental_reconcile_report
            if persisted_incremental_security_report:
                if "incremental_security" not in provision_report:
                    provision_report["incremental_security"] = persisted_incremental_security_report
                if "incremental_security" not in runtime_inventory:
                    runtime_inventory["incremental_security"] = persisted_incremental_security_report

            persisted_incremental_plan = {}
            incremental_plan_path = state_store.stage_artifacts_dir(run.run_id, "provision") / "incremental-execution-plan.json"
            if incremental_plan_path.exists():
                persisted_incremental_plan = json.loads(incremental_plan_path.read_text(encoding="utf-8"))
                artifacts["incremental_execution_plan"] = str(incremental_plan_path)
                if "incremental_execution_plan" not in provision_report:
                    provision_report["incremental_execution_plan"] = persisted_incremental_plan
                if "incremental_execution_plan" not in runtime_inventory:
                    runtime_inventory["incremental_execution_plan"] = persisted_incremental_plan
            ...
            if enforce_a2_4_incremental_entry_gate and isinstance(persisted_incremental_plan, dict):
                persisted_replay_fingerprint = str(
                    persisted_incremental_plan.get("incremental_replay_fingerprint", "")
                ).strip().lower()
                if persisted_replay_fingerprint and persisted_replay_fingerprint != incremental_replay_fingerprint:
            ...
            if enforce_a2_4_incremental_entry_gate and isinstance(persisted_incremental_plan, dict):
                ...
                for artifact_key, artifact_path in required_incremental_artifacts.items():
                ...
                if isinstance(persisted_incremental_plan, dict) and isinstance(expected_correlation, dict) and expected_correlation:
                    if "correlation" not in persisted_incremental_plan:
                        persisted_incremental_plan["correlation"] = dict(expected_correlation)
                    if (
                        ...
                        persisted_incremental_reconcile_report["correlation"] = dict(expected_correlation)
                if expected_correlation:
                    ...
                    if isinstance(persisted_verify_report, dict):
                        verify_correlation = (
                            ...
                            if isinstance(persisted_verify_report.get("correlation"), dict)
                            else {}
                        )
                        for field_name in (
                            "change_id",
                            ...
                            expected_value = str(expected_correlation.get(field_name, "")).strip().lower()
                            current_value = str(verify_correlation.get(field_name, "")).strip().lower()
                            if expected_value != current_value:
                                short_circuit_immutability_issues.append(
                                    ProvisionIssue(
                                        level="error",
    ...
    for issue_payload in observed_state_result.issues:
        ...
        issues.append(
            ProvisionIssue(
                level=str(issue_payload.get("level", "warning")).strip().lower() or "warning",
    ...
    for host_id in observed_state_result.parseable_hosts:
        observed_components = observed_state_result.observed_components_by_host.get(host_id, [])
        host_state = normalized_runtime_state.setdefault(host_id, _normalize_host_state({}))
        host_state["components"] = [
            {
                ...
                "name": str(item.get("name", "")).strip(),
                "image": str(item.get("image", "")).strip(),
                "ports": sorted(int(port) for port in (item.get("ports") or []) if str(port).isdigit()),
                "status": str(item.get("status", "")).strip().lower(),
            }
    ...
    try:
        entries_list = provision_execution_plan.get("entries") if isinstance(provision_execution_plan.get("entries"), list) else []
            ...
            if ctype in API_RUNTIME_COMPONENT_TYPES:
                try:
                    bootstrap_payload = _materialize_api_runtime_bootstrap_for_entry(
                        ...
                        declared_chaincodes={
                            str(item.get("chaincode_id", "")).strip().lower()
                            for item in (manifest_report.get("normalized_chaincode_runtimes") or [])
                            if isinstance(item, dict)
                        },
                        peer_names_by_org=_api_runtime_peer_names_by_org(manifest_report),
                        default_peer_name=_all_api_peers[0] if _all_api_peers else "",
                        default_orderer_name=_api_runtime_first_orderer_name(manifest_report),
                    )
    ...
    if enforce_a2_4_incremental_entry_gate and a2_4_incremental_gate_result is not None:
        ...
        for issue_payload in (
            ...
            issues.append(
                ProvisionIssue(
                    level=str(issue_payload.get("level", "error")).strip().lower() or "error",
                    code=str(issue_payload.get("code", "a2_4_incremental_plan_issue")).strip().lower(),
                    path=str(issue_payload.get("path", "incremental_execution_plan")).strip() or "incremental_execution_plan",
                    message=str(issue_payload.get("message", "Plano incremental inválido.")).strip(),
                )
            )
    ...
    incremental_plan_blocked = bool(incremental_execution_plan.get("blocked", False)) if incremental_plan_enabled else False
    ...
    if incremental_plan_blocked:
        execution_plan_for_apply = {
            "entries": [],
            "action_summary": {"create": 0, "update": 0, "start": 0, "noop": 0, "verify": 0},
            "plan_fingerprint": "",
        }
    ...
    _append_provision_flow_checkpoint(
        ...
        details={
            "plan_fingerprint": str(execution_plan_for_apply.get("plan_fingerprint", "")).strip().lower(),
    ...
    for host_ref in sorted(nodes_by_host.keys()):
        ...
        if host_state["provider"] != "external-vm" or host_state["os_family"] != "linux":
            issues.append(
                ProvisionIssue(
                    level="error",
        ...
        try:
            ...
            allocated_ports: Dict[int, str] = dict(host_state.get("allocated_ports", {}))
            ...
            for item in (host_state.get("components") or []):
                ...
                if component_name:
                    fabric_component_name_index.setdefault(component_name.lower(), component_key)
            fabric_component_postchecks: List[Dict[str, Any]] = [
                dict(item)
                for item in (host_state.get("component_postchecks") or [])
                if isinstance(item, dict)
            ]
            directory_permissions: Dict[str, str] = dict(host_state.get("directory_permissions", {}))
            secret_references: Set[str] = set(host_state.get("secret_references", []))
            crypto_services: Dict[str, Dict[str, Any]] = dict(host_state.get("crypto_services", {}))
            msp_tls_artifacts: Dict[str, Dict[str, Any]] = {
                str(path).strip(): dict(artifact)
                for path, artifact in sorted((host_state.get("msp_tls_artifacts") or {}).items(), key=lambda item: str(item[0]))
                ...
                try:
                    ...
                    if skip_remote_execution:
                        ssh_execution_units.append(
                            _ssh_unit_with_error_classification_alias(
                                ...
                                "metadata": {
                                    ...
                                    "action_reason": str(entry.get("action_reason", "")).strip().lower(),
                        ...
                        unit_key = build_ssh_unit_idempotency_key(
                            ...
                            component_id=component_id,
                            operation=operation,
                            component_signature=str(entry.get("desired_signature", "")).strip(),
                        )
                        ...
                        unit_result = resolved_ssh_executor.execute_unit(
                            ...
                            command=command,
                            metadata={
                                "component_type": str(entry.get("component_type", "")).strip().lower(),
                                "component_name": str(entry.get("name", "")).strip(),
                                "desired_state": str(entry.get("desired_state", "")).strip().lower(),
                                "criticality": str(entry.get("criticality", "")).strip().lower(),
                                "action_reason": str(entry.get("action_reason", "")).strip().lower(),
                        ...
                        ssh_execution_units.append(_ssh_unit_with_error_classification_alias(unit_result.to_dict()))
                    ...
                    if _is_managed_runtime_component_type(component_type):
                        ...
                        existing_key_by_name = fabric_component_name_index.get(component_name_key, "")
                        existing_component = (
                        ...
                        if not existing_component and existing_key_by_name:
                            existing_component = dict(fabric_component_map.get(existing_key_by_name, {}))

                        existing_component_id = str(existing_component.get("component_id", "")).strip()
                        existing_component_name = str(existing_component.get("name", "")).strip()
                        if (
                            existing_component
                    ...
                    if mutating_action and host_storage_path and host_storage_path not in volumes:
                    ...
                    for port in expected_ports:
                        ...
                        elif owner and owner in allowed_owner_ids and owner != component_key:
                            allocated_ports[port] = component_key
                    ...
                    existing_status = str(existing_component.get("status", "")).strip().lower()
                    ...
                    template_healthcheck_available = bool(template.get("healthcheck_available", False))
                    if mutating_action:
                        published_ports = expected_ports
                        component_image = str(template.get("image", "")).strip()
                        component_command = str(template.get("runtime_command", "")).strip()
                        component_env_profile = str(template.get("env_profile", "")).strip().lower()
                        component_storage_profile = str(template.get("storage_profile", "")).strip()
                        component_resources = _normalize_resource_contract(template.get("resources", {}))
                        healthcheck_available = template_healthcheck_available
                        healthcheck_command = str(template.get("healthcheck_command", "")).strip()
                    else:
                        published_ports = _sorted_unique_positive_ints(existing_component.get("ports", [])) or expected_ports
                        component_image = str(existing_component.get("image", "")).strip() or str(template.get("image", "")).strip()
                        component_command = (
                            str(existing_component.get("command", "")).strip()
                            or str(template.get("runtime_command", "")).strip()
                        )
                        component_env_profile = (
                            str(existing_component.get("env_profile", "")).strip().lower()
                        ...
                        if component_resources == {"cpu": 0, "memory_mb": 0, "disk_gb": 0}:
                            component_resources = _normalize_resource_contract(template.get("resources", {}))
                        healthcheck_available = bool(existing_component.get("healthcheck_available", template_healthcheck_available))
                        healthcheck_command = (
                            str(existing_component.get("healthcheck_command", "")).strip()
                            or str(template.get("healthcheck_command", "")).strip()
                        )

                    if healthcheck_available:
                        health_status = "healthy" if component_status == "running" else "unhealthy"
                    else:
                        health_status = "n/a"

                    endpoint_port = published_ports[0] if published_ports else 0
                    endpoint = f"{host_ref}:{endpoint_port}" if endpoint_port > 0 else host_ref
                    endpoint_operational = bool(component_status == "running" and endpoint_port > 0)
                    ...
                    component_payload = {
                        ...
                        "healthcheck_command": sanitize_sensitive_text(healthcheck_command),
                        "service_context": service_context,
                        "endpoint": endpoint,
                    ...
                    for map_key, map_value in fabric_component_map.items():
                        map_component_id = str(map_value.get("component_id", "")).strip()
                    ...
                    for map_key in sorted(keys_to_remove):
                        ...
                        if removed_name_key and fabric_component_name_index.get(removed_name_key) == map_key:
                    ...
                    if component_name_key:
                        ...
                        fabric_component_postchecks = [
                            ...
                            if str(item.get("component_id", "")).strip() != component_key
                            and str(item.get("name", "")).strip().lower() != component_name_key
                        ]
                        fabric_component_postchecks.append(postcheck)
                        if not bool(postcheck.get("ok", False)):
                            level = "error" if desired_state == "required" else "warning"
                            issues.append(
                                ProvisionIssue(
                                    level=level,
            ...
            for runtime_entry in _runtime_bootstrap_entries_for_host(chaincode_runtime_bootstrap_plan, host_ref):
                runtime_name = str(runtime_entry.get("runtime_name", "")).strip()
                if not runtime_name:
                    continue
                target_peer = str(runtime_entry.get("target_peer", "")).strip().lower()
                target_peer_payload = fabric_component_map.get(target_peer, {})
                target_peer_status = str(target_peer_payload.get("status", "")).strip().lower()
                ...
                runtime_entry_fingerprint = str(
                    runtime_entry.get("runtime_bundle_entry_fingerprint", "")
                ).strip().lower()
                ...
                runtime_unit_result = resolved_ssh_executor.execute_unit(
                    ...
                    metadata={
                        ...
                        "runtime_dir": str(runtime_entry.get("runtime_dir", "")).strip(),
                    },
                )
                runtime_unit_payload = _ssh_unit_with_error_classification_alias(runtime_unit_result.to_dict())
                ...
                running = bool(verify_status.get("running", False)) and runtime_verify_capture.result.status == "completed"
                health = str(verify_status.get("health", "unknown")).strip().lower() or "unknown"
                restart_count = max(int(verify_status.get("restart_count", 0) or 0), 0)

                endpoint_ports = _sorted_unique_positive_ints(runtime_entry.get("endpoint_ports", []))
                ...
                if endpoint_ports:
                    ...
                    smoke_unit = resolved_ssh_executor.execute_unit(
                        ...
                        command=_runtime_smoke_check_command(runtime_name=runtime_name, endpoint_port=smoke_port),
                        metadata={
                            "runtime_name": runtime_name,
                            "target_peer": target_peer,
                            "channel_id": str(runtime_entry.get("channel_id", "")).strip().lower(),
                    ...
                    smoke_payload = _ssh_unit_with_error_classification_alias(smoke_unit.to_dict())
                    runtime_verify_units.append(smoke_payload)
                    ssh_execution_units.append(smoke_payload)
                    smoke_check = {
                        "status": "passed" if smoke_unit.status == "completed" else "failed",
                        "reason": "",
                        "port": smoke_port,
                        "error_classification": str(
                            smoke_payload.get("error_classification", smoke_payload.get("classification", ""))
                        ).strip().lower(),
                    }

                health_ok = health in {"healthy", "n/a", "unknown"}
                smoke_ok = str(smoke_check.get("status", "")).strip().lower() in {"passed", "skipped"}
                ...
                if runtime_ok:
                    ...
                    acao_recomendada = "Prosseguir com reconciliação e verify final do run." 
                else:
                    causa = (
                ...
                if not runtime_ok:
                    ...
                    try:
                        component_type = str(runtime_entry.get("component_type", "")).strip().lower()
                        if resolved_ssh_executor is not None and state_store is not None and (
                            component_type in API_RUNTIME_COMPONENT_TYPES
                            or component_type in {"api_gateway", "chaincode_gateway", "gateway"}
                        ):
                            try:
                    ...
                    declared_chaincode = str(runtime_entry.get("chaincode_id", "")).strip().lower()
                    declared_version = str(runtime_entry.get("version", "")).strip().lower() or "1.0"
                    if declared_chaincode:
                        ...
                        try:
                            comp_type = _canonical_component_type(runtime_entry.get("component_type", "") or "")
                        except Exception:
                            comp_type = ""
            ...
            for profile in bootstrap_by_host.get(host_ref, []):
                ...
                tls_ca_credential_ref = str(storage.get("tls_ca_credential_ref", "")).strip()
                ...
                if not ca_credential_ref or not tls_ca_credential_ref or ca_credential_ref == tls_ca_credential_ref:
                    issues.append(
                        ProvisionIssue(
                            level="error",
                ...
                if not private_key_store.startswith(AUTHORIZED_PRIVATE_KEY_STORE_PREFIX):
                    issues.append(
                        ProvisionIssue(
                            level="error",
                ...
                root_subject = f"CN=RootCA.{domain}" if domain else f"CN=RootCA.{org_id}"
                intermediate_subject = (
                    f"CN=IntermediateCA.{domain}" if domain else f"CN=IntermediateCA.{org_id}"
                )
                tls_root_subject = f"CN=TLSRootCA.{domain}" if domain else f"CN=TLSRootCA.{org_id}"
                tls_intermediate_subject = (
                    f"CN=TLSIntermediateCA.{domain}" if domain else f"CN=TLSIntermediateCA.{org_id}"
                )

                existing_service = crypto_services.get(org_id, {})
                existing_issued_identities = [
                    item for item in (existing_service.get("issued_identities") or []) if isinstance(item, dict)
                ]
                existing_issuance_events = [
                    item for item in (existing_service.get("issuance_events") or []) if isinstance(item, dict)
                ]
            ...
            for target in identity_targets_by_host.get(host_ref, []):
                ...
                if not policy_allowed:
                    issues.append(
                        ProvisionIssue(
                            ...
                            path=policy_path or f"runtime_state.{host_ref}.crypto_services.{org_id}",
                            message=(
                ...
                events = [item for item in (service.get("issuance_events") or []) if isinstance(item, dict)]
                id_key = _identity_id(org_id=org_id, identity_type=identity_type, logical_subject=logical_subject)
                key_size = int(ca_info.get("key_size", 0))
                validity_days = int(ca_info.get("validity_days", 0)) if str(ca_info.get("validity_days", "")).isdigit() else 365
                algorithm = str(ca_info.get("algorithm", "")).strip().lower()

                related = [item for item in identities if str(item.get("identity_id", "")).strip().lower() == id_key]
                active = [item for item in related if str(item.get("status", "")).strip().lower() == "active"]
                if len(active) > 1:
                    serials = sorted({str(item.get("serial", "")).strip() for item in active if str(item.get("serial", "")).strip()})
                    issues.append(
                        ProvisionIssue(
                            level="error",
                ...
                elif _should_reenroll_identity(current_active, desired_material, key_size):
            ...
            inject_chain_mismatch = bool(host_state.get("inject_msp_tls_chain_mismatch", False))
            inject_key_mismatch = bool(host_state.get("inject_msp_tls_key_mismatch", False))
            for node in host_nodes:
                node_id = str((node or {}).get("node_id", "")).strip()
                node_type = str((node or {}).get("node_type", "")).strip().lower()
                org_id = str((node or {}).get("org_id", "")).strip().lower()
                ...
                if not bool(((manifest.get("validation") or {}).get("chain_valid", False))):
                    issues.append(
                        ProvisionIssue(
                            level="error",
                ...
                if not bool(((manifest.get("validation") or {}).get("key_cert_match", False))):
                    issues.append(
                        ProvisionIssue(
                            level="error",
            ...
            if a2_manifest_mode:
                active_required_fabric_components = [
                    ...
                    if isinstance(item, dict)
                    and _is_fabric_base_component_type(str(item.get("component_type", "")).strip().lower())
            ...
            if host_state.get("fail_provision", False):
                raise RuntimeError(f"Falha injetada de provisionamento no host '{host_ref}'.")
            ...
            host_state["allocated_ports"] = dict(sorted(allocated_ports.items(), key=lambda item: item[0]))
            host_state["nodes"] = dict(sorted(nodes.items(), key=lambda item: item[0]))
            host_state["directory_permissions"] = {
                path: directory_permissions[path] for path in sorted(directory_permissions.keys())
            }
            host_state["secret_references"] = sorted(secret_references)
            host_state["crypto_services"] = {
                org_id: crypto_services[org_id] for org_id in sorted(crypto_services.keys())
            }
            ...
            host_state["msp_tls_manifests"] = sorted(
                [msp_tls_manifest_map[node_id] for node_id in sorted(msp_tls_manifest_map.keys())],
                key=lambda item: (
                    str(item.get("org_id", "")),
                    str(item.get("node_id", "")),
                    str(item.get("node_type", "")),
            ...
            host_state["components"] = sorted(
                [dict(item) for item in fabric_component_map.values() if isinstance(item, dict)],
                key=lambda item: (
                    str(item.get("component_type", "")),
                    str(item.get("component_id", "")),
                    str(item.get("name", "")),
            ...
            host_state["component_postchecks"] = sorted(
                [dict(item) for item in fabric_component_postchecks if isinstance(item, dict)],
                key=lambda item: (
                    str(item.get("component_type", "")),
                    str(item.get("component_id", "")),
                    str(item.get("name", "")),
            ...
            compensation_events.append(
                {
                    ...
                    "incremental_context": {
                        ...
                        "incremental_pairings": (
                            [
                                dict(item)
                                for item in (incremental_execution_plan.get("incremental_peer_couch_pairs") or [])
                                if isinstance(item, dict)
                            ]
                        ...
                        "incremental_orderer_governance_updates": (
                            [
                                dict(item)
                                for item in (
                                    incremental_execution_plan.get("incremental_orderer_governance_updates") or []
                                )
                                if isinstance(item, dict)
                            ]
                            if isinstance(incremental_execution_plan, dict)
                        ...
                        "incremental_channel_association_projections": (
                            [
                                dict(item)
                                for item in (
                                    incremental_execution_plan.get("incremental_channel_association_projections") or []
                                )
                                if isinstance(item, dict)
                            ]
                            if isinstance(incremental_execution_plan, dict)
                        ...
                        "incremental_channel_step_dependencies": (
                            [
                                dict(item)
                                for item in (
                                    incremental_execution_plan.get("incremental_channel_step_dependencies") or []
                                )
                                if isinstance(item, dict)
                            ]
                            if isinstance(incremental_execution_plan, dict)
    ...
    chaincode_runtime_template_contract_public = _runtime_template_contract_public_projection(
    ...
    incremental_orderer_governance_updates = (
        [
            dict(item)
            for item in (incremental_execution_plan.get("incremental_orderer_governance_updates") or [])
            if isinstance(item, dict)
        ]
        if isinstance(incremental_execution_plan, dict)
        else []
    ...
    incremental_channel_association_projections = (
        [
            dict(item)
            for item in (incremental_execution_plan.get("incremental_channel_association_projections") or [])
            if isinstance(item, dict)
        ]
        if isinstance(incremental_execution_plan, dict)
        else []
    ...
    incremental_channel_step_dependencies = (
        [
            dict(item)
            for item in (incremental_execution_plan.get("incremental_channel_step_dependencies") or [])
            if isinstance(item, dict)
        ]
        if isinstance(incremental_execution_plan, dict)
        else []
    ...
    channel_association_scope = (
        ...
        if isinstance(incremental_execution_plan, dict)
        and isinstance(incremental_execution_plan.get("channel_association_scope"), dict)
        else {
    ...
    channel_association_summary = {
        ...
        "out_of_scope_channel_ids": sorted(
            {
                str(item).strip().lower()
                for item in (channel_association_scope.get("out_of_scope_channel_ids") or [])
                if str(item).strip()
            }
        ),
        "org_scope_channel_ids": sorted(
            {
                str(item).strip().lower()
                for item in (channel_association_scope.get("org_scope_channel_ids") or [])
                if str(item).strip()
            }
        ),
        "requested_channel_ids": sorted(
            {
                str(item).strip().lower()
                for item in (channel_association_scope.get("requested_channel_ids") or [])
                if str(item).strip()
            }
        ),
        "projected_channel_ids": sorted(
            {
                str(item).strip().lower()
                for item in (channel_association_scope.get("projected_channel_ids") or [])
                if str(item).strip()
            }
        ),
    ...
    if incremental_plan_enabled and isinstance(incremental_execution_plan, dict) and incremental_execution_plan:
        ...
        required_new_non_converged_rows = (
            ...
            if isinstance(incremental_reconcile_report.get("required_new_non_converged"), list)
            else []
        )
        ...
        incremental_operational_continuity_report = _materialize_incremental_operational_continuity_report(
            run=run,
            incremental_execution_plan=incremental_execution_plan,
            observed_state_baseline=observed_state_baseline,
            normalized_runtime_state=normalized_runtime_state,
        ...
        continuity_regressions = (
            ...
            if isinstance(incremental_operational_continuity_report.get("availability_regressions"), list)
            else []
        )
        ...
        out_of_scope_unjustified_rows = (
            ...
            if isinstance(incremental_reconcile_report.get("out_of_scope_unjustified"), list)
            else []
        )
        ...
        incremental_operational_continuity_report_public = _sanitize_incremental_evidence_payload(
            incremental_operational_continuity_report
        )
        incremental_security_report = _materialize_incremental_security_report(
            ...
            connection_profile_registry=connection_profile_registry,
            secret_vault_registry=secret_vault_registry,
            enforce_credential_reference_validation=bool(enforce_credential_reference_validation),
        )

        secure_ref_violations = (
            incremental_security_report.get("secure_ref_violations")
            if isinstance(incremental_security_report.get("secure_ref_violations"), list)
            else []
        )
        for index, row in enumerate(secure_ref_violations):
            ...
            issues.append(
                ProvisionIssue(
                    level="error",
                    code=str(row.get("code", "a2_4_incremental_secure_ref_missing")).strip().lower()
                    or "a2_4_incremental_secure_ref_missing",
                    path=str(row.get("path", "incremental_security_report.secure_ref_violations")).strip()
                    or f"incremental_security_report.secure_ref_violations[{index}]",
                    message=str(row.get("message", "Referência segura obrigatória ausente para expansão incremental.")).strip(),
                )
            )

        secret_literal_exposures = (
            incremental_security_report.get("secret_literal_exposures")
            if isinstance(incremental_security_report.get("secret_literal_exposures"), list)
            else []
        )
        for index, row in enumerate(secret_literal_exposures):
            ...
            issues.append(
                ProvisionIssue(
                    ...
                    path=str(row.get("path", "incremental_security_report.secret_literal_exposures")).strip()
    ...
    if not incremental_execution_plan_public and isinstance(incremental_execution_plan, dict) and incremental_execution_plan:
        incremental_execution_plan_public = _sanitize_incremental_evidence_payload(incremental_execution_plan)
    if not incremental_reconcile_report_public and isinstance(incremental_reconcile_report, dict) and incremental_reconcile_report:
        incremental_reconcile_report_public = _sanitize_incremental_evidence_payload(incremental_reconcile_report)
    if (
    ...
    first_error_issue = next((issue for issue in issues if issue.level == "error"), None)
    ...
    actionable_issues = [
        _actionable_issue_payload(issue, stage="provision")
        for issue in issues
    ]
    ...
    ssh_execution_inline = {
        ...
        "generated_at": str(execution_plan.get("generated_at", "")).strip() or run.started_at or utc_now_iso(),
    }
    ...
    runtime_reconcile_report = {
        **runtime_reconcile_inline,
        ...
        "rows_detailed": (
            [dict(item) for item in runtime_reconcile_report.get("rows", []) if isinstance(item, dict)]
            if isinstance(runtime_reconcile_report.get("rows"), list)
            else []
        ),
        "required_non_converged": (
        ...
        for _item in (
            ...
            try:
                if not isinstance(_item, dict):
                    continue
                expected = _item.get("expected") if isinstance(_item.get("expected"), dict) else {}
                observed = _item.get("observed") if isinstance(_item.get("observed"), dict) else {}
                host_id = str(_item.get("host_id", "")).strip()
                runtime_name = str(_item.get("runtime_name", "")).strip()
                comp_type = _canonical_component_type(str(observed.get("component_type", "") or expected.get("component_type", "") or ""))
                if comp_type in API_RUNTIME_COMPONENT_TYPES and host_id and runtime_name:
                    try:
                        _probe_identities_and_restart_if_changed(
                            run=run,
                            state_store=state_store,
    ...
    if isinstance(incremental_execution_plan_public, dict) and incremental_execution_plan_public:
        incremental_execution_plan_public = {
            **incremental_execution_plan_public,
            "topology_generation": int(incremental_origin_metadata.get("topology_generation", 0) or 0),
            "correlation": artifact_correlation,
        }
    if isinstance(incremental_reconcile_report_public, dict) and incremental_reconcile_report_public:
    ...
    verify_report_wp_a22 = {
        ...
        "reason_codes": [str(item.get("code", "")).strip().lower() for item in decision_reasons if str(item.get("code", "")).strip()],
        "summary": {
            "error_count": error_count,
            "warning_count": warning_count,
    ...
    stage_reports_wp_a22 = {
        ...
        "reports": {
            "prepare": {
                "source_stage": str(execution_plan.get("stage", "")).strip().lower() or "prepare",
    ...
    runtime_inventory = {
        ...
        "idempotency_semantics": {
            "stage_idempotency_key": run.idempotency_key("provision"),
            "plan_fingerprint": str(provision_execution_plan.get("plan_fingerprint", "")).strip().lower(),
            "mutating_actions": {
    ...
    provision_report = {
        ...
        "fabric_base_provisioning": {
            ...
            "fabric_component_count": sum(
                ...
                if _is_fabric_base_component_type(str((component or {}).get("component_type", "")).strip().lower())
            ),
            "postcheck_total": sum(
                1
                for host in runtime_inventory["hosts"]
            ...
            "peer_couch_pairing_ok_hosts": sorted(
                [
                    ...
                    for host in runtime_inventory["hosts"]
                    if bool(((host.get("fabric_base_summary") or {}).get("peer_couch_pairing_ok", False)))
                ]
            ),
        },
        "api_provisioning": {
            "api_component_count": sum(
                ...
                for host in runtime_inventory["hosts"]
                for component in (host.get("components") or [])
                if _is_api_runtime_component_type(str((component or {}).get("component_type", "")).strip().lower())
            ),
            "active_api_component_count": sum(
                ...
                for component in (host.get("components") or [])
                if _is_api_runtime_component_type(str((component or {}).get("component_type", "")).strip().lower())
                and str((component or {}).get("status", "")).strip().lower() == "running"
            ),
            ...
            "service_context_declared_count": sum(
                ...
                for component in (host.get("components") or [])
                if _is_api_runtime_component_type(str((component or {}).get("component_type", "")).strip().lower())
                and bool(_normalized_service_context((component or {}).get('service_context', {})))
            ),
        },
        "failure_handling": {
    ...
    if state_store is not None:
        ...
        inventory_path = state_store.write_artifact(
            ...
            stage="provision",
            artifact_name="runtime-inventory.json",
            content=json.dumps(runtime_inventory, ensure_ascii=False, indent=2, sort_keys=True).encode("utf-8"),
        )
        report_path = state_store.write_artifact(
            run_id=run.run_id,
            stage="provision",
            artifact_name="provision-report.json",
            content=json.dumps(provision_report, ensure_ascii=False, indent=2, sort_keys=True).encode("utf-8"),
        )
        artifacts = {
            ...
            input_hash=payload_sha256(
                {
                    ...
                    "incremental_execution_plan": {
                        ...
                        "incremental_replay_fingerprint": str(
                            incremental_execution_plan.get("incremental_replay_fingerprint", "")
                        ).strip().lower(),
                        "entry_count": int(incremental_execution_plan.get("entry_count", 0) or 0),
                        "action_summary": dict(incremental_execution_plan.get("action_summary", {})),
                    },
                    "incremental_reconcile": {
                        "incremental_reconcile_fingerprint": str(
                            incremental_reconcile_report_public.get("incremental_reconcile_fingerprint", "")
                        ).strip().lower(),
                        "summary": dict(incremental_reconcile_report_public.get("summary", {})),
                        "required_new_non_converged": list(
                            incremental_reconcile_report_public.get("required_new_non_converged", [])
                        ),
                        "out_of_scope_unjustified": list(
                            incremental_reconcile_report_public.get("out_of_scope_unjustified", [])
                        ),
                    },
                    "incremental_operational_continuity": {
                        "operational_continuity_fingerprint": str(
                            incremental_operational_continuity_report_public.get("operational_continuity_fingerprint", "")
                        ).strip().lower(),
                        "summary": dict(incremental_operational_continuity_report_public.get("summary", {})),
                        "availability_regressions": list(
                            incremental_operational_continuity_report_public.get("availability_regressions", [])
                        ),
                        "stage_impact": dict(incremental_operational_continuity_report_public.get("stage_impact", {})),
                    },
                    "incremental_security": {
                        "incremental_security_fingerprint": str(
                            incremental_security_report.get("incremental_security_fingerprint", "")
                        ).strip().lower(),
                    ...
                    "chaincode_runtime_template_contract": {
                        ...
                        "summary": dict(chaincode_runtime_template_contract.get("summary", {})),
                        "catalog": dict(chaincode_runtime_template_contract.get("catalog", {})),
                    },
                    "chaincode_runtime_bundle": {
                        "runtime_bundle_fingerprint": str(
                            chaincode_runtime_bundle_report.get("runtime_bundle_fingerprint", "")
                        ).strip().lower(),
                        "summary": dict(chaincode_runtime_bundle_report.get("summary", {})),
                    },
                    "chaincode_runtime_bootstrap": {
                        "runtime_bootstrap_plan_fingerprint": str(
                            chaincode_runtime_bootstrap_plan.get("runtime_bootstrap_plan_fingerprint", "")
                    ...
                    "chaincode_runtime_reconcile": {
                        "runtime_reconcile_fingerprint": str(
                            runtime_reconcile_inline.get("runtime_reconcile_fingerprint", "")
                        ).strip().lower(),
                        "summary": dict(runtime_reconcile_inline.get("summary", {})),
                        "rows": list(runtime_reconcile_inline.get("rows", [])),
                    },
                    "observed_state_baseline_summary": observed_state_baseline.get("summary", {}),
                    "reconciliation_summary": {
                        "divergence_summary": reconciliation_plan.get("divergence_summary", {}),
                        "action_summary": reconciliation_plan.get("action_summary", {}),
                    },
                    "ssh_execution_summary": ssh_execution_report.get("summary", {}),
                    "concurrency_control_summary": concurrency_control_report.get("summary", {}),
                    "stage_checkpoint_summary": stage_checkpoint_report.get("summary", {}),
    ...
    try:
        declared_workarounds = execution_plan.get("workarounds") if isinstance(execution_plan.get("workarounds"), list) else []
        for w in declared_workarounds:
            try:
                entry = {
                    "declared_at": str(execution_plan.get("generated_at", utc_now_iso())),
                    "workaround": w,
                }
    ...
    try:
        image_patches = execution_plan.get("image_patches") if isinstance(execution_plan.get("image_patches"), list) else []
    ...
    *,
    ...
    try:
        ...
        if channel_name in list_out:
            state_store.write_text_artifact(run_id, "provision", f"gateway-{host_id}-{component_id}-channel-join-{channel_name}.txt", "already joined")
            issues.append(
                ProvisionIssue(level="info", code="verify_channel_already_joined", path=f"runtime_state.{host_id}.components.{component_id}", message="channel already present on peer", runtime_name=component_id)
            )
            joined = True
        ...
        if "Saved to" in fetch_out or "Saved" in fetch_out:
            ...
            if "EXISTS" in blockcheck_out:
                ...
                if "Join successful" in join_out or "joined" in join_out.lower():
                    state_store.write_text_artifact(run_id, "provision", f"gateway-{host_id}-{component_id}-channel-join-{channel_name}.txt", join_out)
                    issues.append(
                        ProvisionIssue(level="info", code="verify_channel_join_applied", path=f"runtime_state.{host_id}.components.{component_id}", message="peer joined channel", runtime_name=component_id)
                    )
                    joined = True
        else:
            ...
            if "Saved to" in participation_out or "Saved" in participation_out:
                ...
                try:
                    state_store.write_text_artifact(run_id, "provision", f"gateway-{host_id}-{component_id}-channel-participation-fetch-{channel_name}.txt", participation_out)
                except Exception:
                    pass
                ...
                if "EXISTS" in str(getattr(blockcheck_after, "stdout", "") or ""):
                    ...
                    if "Join successful" in join_out or "joined" in join_out.lower():
                        state_store.write_text_artifact(run_id, "provision", f"gateway-{host_id}-{component_id}-channel-join-{channel_name}.txt", join_out)
                        issues.append(
                            ProvisionIssue(level="info", code="verify_channel_join_applied", path=f"runtime_state.{host_id}.components.{component_id}", message="peer joined channel via participation fetch", runtime_name=component_id)
                        )
                        joined = True
        ...
        try:
            orderer_sha = (msp_fetch_out.split()[0] if msp_fetch_out and msp_fetch_out.split() else "").strip()
            local_sha = (msp_local_out.split()[0] if msp_local_out and msp_local_out.split() else "").strip()
            if orderer_sha and local_sha and orderer_sha != local_sha:
                try:
                    artifact_name = f"gateway-{host_id}-{component_id}-msp-align-{channel_name}.txt"
                    state_store.write_text_artifact(run_id, "provision", artifact_name, msp_align_out)
                except Exception:
                    pass
                issues.append(
                    ProvisionIssue(level="warning", code="verify_msp_mismatch", path=f"runtime_state.{host_id}.components.{component_id}", message="msp mismatch detected between orderer and peer", runtime_name=component_id)
                )
        ...
        if "CA_FOUND" in ca_out or "CA_FOUND" in ca_out.upper():
            ...
            join_out = str(getattr(join_op, "stdout", "") or "")
            if "Join successful" in join_out or "joined" in join_out.lower():
                artifact_name = f"gateway-{host_id}-{component_id}-msp-regen-{channel_name}.txt"
                state_store.write_text_artifact(run_id, "provision", artifact_name, regen_out)
                issues.append(
                    ProvisionIssue(level="info", code="verify_msp_regenerated", path=f"runtime_state.{host_id}.components.{component_id}", message="msp regenerated and join reattempted", runtime_name=component_id)
                )
                issues.append(
                    ProvisionIssue(level="info", code="verify_channel_join_applied", path=f"runtime_state.{host_id}.components.{component_id}", message="peer joined channel after regen", runtime_name=component_id)
                )
                joined = True
        ...
        try:
            ...
            if "Consenter joined" in consenter_out or "Consenter joined successfully" in consenter_out:
                try:
                    artifact_name = f"gateway-{host_id}-{component_id}-orderer-consenter-join-{channel_name}.txt"
                    state_store.write_text_artifact(run_id, "provision", artifact_name, consenter_out)
                except Exception:
                    pass
                issues.append(
                    ProvisionIssue(level="info", code="verify_orderer_consenter_join_applied", path=f"runtime_state.{host_id}.components.{component_id}", message="orderer consenter join applied", runtime_name=component_id)
                )
                joined = True
        except Exception:
            pass
        ...
        issues.append(
            ProvisionIssue(level="error", code="verify_channel_block_missing", path=f"runtime_state.{host_id}.components.{component_id}", message="channel block missing and join could not be applied", runtime_name=component_id)
        )
    except Exception as e:
        issues.append(
            ProvisionIssue(level="error", code="provision_host_failure", path=f"runtime_state.{host_id}.components.{component_id}", message=str(e), runtime_name=component_id)
        )
...
def _validate_and_populate_gateway_inventory(
    *,
    ...
    try:
        for host_id, comps in sorted((nodes_by_host or {}).items()):
            for comp in (comps or []):
                comp_type = _canonical_component_type(comp.get("component_type", "") or "")
                comp_id = str(comp.get("component_id") or comp.get("name") or "").strip()
                ...
                payload["gateway_inventory"][key] = {"found": bool(found), **container_info}
                ...
                try:
                    ...
                    state_store.write_text_artifact(run.run_id, "prepare", logs_name, logs_out)
                except Exception:
                    pass
                ...
                if found and actual_ports:
                    ...
                    for port in actual_ports:
                        try:
                            ...
                            if not ("succeeded" in probe_out or "open" in probe_out or "200" in probe_out):
                                port_probe_ok = False
                        except Exception as e:
                    ...
                    try:
                        state_store.write_text_artifact(run.run_id, "prepare", probe_artifact, port_probe_result)
                    except Exception:
                        pass
                    event_code = "prepare_gateway_ok" if port_probe_ok else "prepare_gateway_port_mismatch"
                    event_payload = {
                        "ts": utc_now_iso(),
                        "stage": "prepare",
                        "host_id": host_id,
                    ...
                    if actual_ports:
                        if not port_probe_ok:
                            issues.append(
                                ProvisionIssue(level="error", code="prepare_gateway_port_mismatch", path=f"runtime_state.{host_id}.components.{comp_id}", message="gateway port mismatch or unreachable", runtime_name=comp_id)
                            )
                            checkpoints.append({"stage": "prepare", "status": "failed", "component": comp_id})
                if not found:
                    issues.append(
                        ProvisionIssue(level="error", code="prepare_gateway_container_missing", path=f"runtime_state.{host_id}.components.{comp_id}", message="gateway container missing", runtime_name=comp_id)
                    )
                    checkpoints.append({"stage": "prepare", "status": "failed", "component": comp_id})
    except Exception as e:
        issues.append(ProvisionIssue(level="error", code="provision_host_failure", path="prepare", message=str(e)))

    # persist inventory artifact
    try:
        state_store.write_json_artifact(run.run_id, "prepare", "gateway-inventory.json", payload)
    except Exception:
        pass
    ...
    try:
        ...
        events_file = events_dir / "prepare-events.jsonl"
        with events_file.open("a", encoding="utf-8") as fh:
            fh.write(json.dumps({"ts": utc_now_iso(), "stage": "prepare", "gateway_inventory_count": len(payload.get("gateway_inventory", {}))}) + "\n")
    except Exception:
        pass
    return issues
...
def _ensure_chaincode_lifecycle(
    *,
    ...
    try:
        ...
        state_store.write_text_artifact(run_id, "provision", f"gateway-{host_id}-{component_id}-chaincode-install-{chaincode_name}.txt", install_stdout)
        issues.append(
            ProvisionIssue(level="info", code="lifecycle_chaincode_install_attempted", path=f"runtime_state.{host_id}.components.{component_id}", message="chaincode install attempted", runtime_name=component_id)
        )
        ...
        if package_id:
            state_store.write_text_artifact(run_id, "provision", f"gateway-{host_id}-{component_id}-chaincode-packageid-{chaincode_name}.txt", package_id)
            issues.append(
                ProvisionIssue(level="info", code="lifecycle_chaincode_installed", path=f"runtime_state.{host_id}.components.{component_id}", message=f"package_id={package_id}", runtime_name=component_id)
            )
        else:
            issues.append(
                ProvisionIssue(level="warning", code="lifecycle_chaincode_package_id_missing", path=f"runtime_state.{host_id}.components.{component_id}", message="package id not found after queryinstalled", runtime_name=component_id)
            )

        # 3) approve
        approve_out = ssh_executor.execute_unit_with_output(operation=f"chaincode_approve:{host_id}:{component_id}:{chaincode_name}:{chaincode_version}:{package_id}")
        approve_stdout = str(getattr(approve_out, "stdout", "") or "")
        state_store.write_text_artifact(run_id, "provision", f"gateway-{host_id}-{component_id}-chaincode-approve-{chaincode_name}.txt", approve_stdout)

        # 4) commit
        commit_out = ssh_executor.execute_unit_with_output(operation=f"chaincode_commit:{host_id}:{component_id}:{chaincode_name}:{chaincode_version}")
        commit_stdout = str(getattr(commit_out, "stdout", "") or "")
        state_store.write_text_artifact(run_id, "provision", f"gateway-{host_id}-{component_id}-chaincode-commit-{chaincode_name}.txt", commit_stdout)

        # 5) querycommitted -> verify
        qc_out = ssh_executor.execute_unit_with_output(operation=f"chaincode_querycommitted:{host_id}:{component_id}")
        qc_stdout = str(getattr(qc_out, "stdout", "") or "")
        state_store.write_text_artifact(run_id, "provision", f"gateway-{host_id}-{component_id}-chaincode-querycommitted-{chaincode_name}.txt", qc_stdout)
        ...
        if committed_name == chaincode_name and committed_version == chaincode_version:
            issues.append(
                ProvisionIssue(level="info", code="lifecycle_chaincode_committed", path=f"runtime_state.{host_id}.components.{component_id}", message=f"committed {chaincode_name}:{chaincode_version}", runtime_name=component_id)
            )
        else:
            issues.append(
                ProvisionIssue(level="warning", code="lifecycle_chaincode_not_committed", path=f"runtime_state.{host_id}.components.{component_id}", message="chaincode not found committed on channel(s)", runtime_name=component_id)
            )
    except Exception as e:
        issues.append(
            ProvisionIssue(level="error", code="lifecycle_chaincode_failure", path=f"runtime_state.{host_id}.components.{component_id}", message=str(e), runtime_name=component_id)
        )
...
def _tls_san_inspect_and_apply_hostnameoverride(
    *,
    ...
    try:
        ...
        try:
            state_store.write_text_artifact(run_id, "provision", f"gateway-{host_id}-{component_id}-tls-san-check.txt", stdout)
        except Exception:
            pass
        ...
        issues.append(ProvisionIssue(level="warning", code="verify_gateway_tls_san_missing", path=f"runtime_state.{host_id}.components.{component_id}", message="TLS SAN missing", runtime_name=component_id))
        ...
        try:
            state_store.write_text_artifact(run_id, "provision", f"gateway-{host_id}-{component_id}-hostnameoverride-apply.txt", aout)
        except Exception:
            pass
        if aout and ("PATCHED" in aout or "APPLIED" in aout or aout.strip().upper() == "OK"):
            issues.append(ProvisionIssue(level="info", code="verify_gateway_tls_hostnameoverride_applied", path=f"runtime_state.{host_id}.components.{component_id}", message="hostnameoverride applied", runtime_name=component_id))
        else:
            issues.append(ProvisionIssue(level="warning", code="verify_gateway_tls_hostnameoverride_failed", path=f"runtime_state.{host_id}.components.{component_id}", message="hostnameoverride apply failed", runtime_name=component_id))
    except Exception as e:
        issues.append(ProvisionIssue(level="error", code="provision_host_failure", path=f"runtime_state.{host_id}.components.{component_id}", message=str(e), runtime_name=component_id))
...
def _tls_regenerate_if_ca_available(
    *,
    ...
    try:
        ...
        if "NO_CA" in out:
            issues.append(ProvisionIssue(level="warning", code="verify_gateway_tls_regen_no_ca", path=f"runtime_state.{host_id}.components.{component_id}", message="no local CA available for regen", runtime_name=component_id))
            return
        if "CERT_GENERATED" in out or "CERT_OK" in out:
            try:
                state_store.write_text_artifact(run_id, "provision", f"gateway-{host_id}-{component_id}-tls-regen.txt", out)
            except Exception:
                pass
            issues.append(ProvisionIssue(level="info", code="verify_gateway_tls_regenerated_applied", path=f"runtime_state.{host_id}.components.{component_id}", message="tls regenerated and applied", runtime_name=component_id))
            return
        issues.append(ProvisionIssue(level="warning", code="verify_gateway_tls_regen_failed", path=f"runtime_state.{host_id}.components.{component_id}", message="tls regenerate failed or returned unexpected output", runtime_name=component_id))
    except Exception as e:
        issues.append(ProvisionIssue(level="error", code="provision_host_failure", path=f"runtime_state.{host_id}.components.{component_id}", message=str(e), runtime_name=component_id))
...
def _tls_regenerate_with_san_command(
    ...
    *,
    ...
    san_fragment = ",".join([f"DNS:{s}" if not _is_ip_address(s) else f"IP:{s}" for s in san_list])
    subj_alt = f"subjectAltName={san_fragment}" if san_fragment else ""
    cmd_parts = [
        "openssl genrsa -out {key} 2048".format(key=key_path),
        "openssl req -new -key {key} -out /tmp/csr.pem -subj '/CN={host}'".format(key=key_path, host=cert_path),
...
def _discover_peer_orderer_cert_and_regenerate(
    *,
    ...
    try:
        ...
        if not lines:
            issues.append(ProvisionIssue(level="info", code="verify_gateway_peer_orderer_tls_not_found", path=f"runtime_state.{host_id}.components.{component_id}", message="no peer/orderer tls pair discovered", runtime_name=component_id))
            return
        idx = 1
        for ln in lines:
            ...
            try:
                artifact_name = f"gateway-{host_id}-{component_id}-peer{idx}-tls-regen.txt"
                state_store.write_text_artifact(run_id, "provision", artifact_name, f"regen:{cert_path}")
            except Exception:
                pass
            idx += 1
        # persist a discovery summary artifact expected by integration tests
        try:
            state_store.write_text_artifact(run_id, "provision", f"gateway-{host_id}-{component_id}-peer-tls-discovery.txt", out)
        except Exception:
            pass
    except Exception as e:
        issues.append(ProvisionIssue(level="error", code="provision_host_failure", path=f"runtime_state.{host_id}.components.{component_id}", message=str(e), runtime_name=component_id))