#!/usr/bin/env bash
set -euo pipefail
RUN_ID="${1:?usage: bootstrap_run_channel_chaincode.sh <run_id>}"
...
[ -s "/tmp/cognus/${CHANNEL}.block" ] || { echo "[bootstrap] channel block missing" >&2; tail -n 80 /tmp/cognus/configtxgen.log >&2 || true; exit 21; }
...
docker exec "$PEER" sh -lc "peer channel list"
...
[ -n "$PKG_ID" ] || { echo "[bootstrap] package id missing" >&2; docker exec "$PEER" sh -lc "peer lifecycle chaincode queryinstalled 2>/dev/null || true"; exit 31; }
...
docker exec "$PEER" sh -lc "peer lifecycle chaincode checkcommitreadiness --channelID ${CHANNEL} --name ${CC_NAME} --version ${CC_VERSION} --sequence 1 --output json >/tmp/cc_ready.json 2>&1 || true; tail -n 80 /tmp/cc_ready.json"
