...
def init_env_vars(node, org):
    ...
    runtime_base = os.path.join(CELLO_HOME, "hyperledger", "fabric")
    runtime_orderer_dir = os.path.join(runtime_base, f"{node.name}.{org_domain}")
    runtime_peer_dir = os.path.join(runtime_base, f"{node.name}.{org_name}")
    ...
    if node.type == "orderer":
        tlsca_name = f"tlsca.{org_domain}-cert.pem"
        legacy_orderer_dir = os.path.join(
            dir_certificate, "orderers", f"{node.name}.{org_domain}"
        )
        admin_tls_dir = os.path.join(
            dir_certificate, "users", f"Admin@{org_domain}", "tls"
        )
        # Use admin TLS cert/key when available. Some node records persist the
        # enrollment MSP cert/key, which are not accepted by osnadmin. Always
        # prefer artifacts signed by the TLS CA before falling back to MSP or
        # server credentials.
        admin_cert = getattr(node, "admin_cert", None)
        admin_key = getattr(node, "admin_key", None)

        def _admin_candidate(path, expected_segment):
            return path if path and expected_segment in path else None

        envs = {
            "CORE_PEER_TLS_ENABLED": "true",
            "ORDERER_CA": _first_existing(
                [
                    os.path.join(runtime_orderer_dir, "tls", "ca.crt"),
                    os.path.join(runtime_orderer_dir, "msp", "tlscacerts", tlsca_name),
                    os.path.join(legacy_orderer_dir, "msp", "tlscacerts", tlsca_name),
                    os.path.join(dir_certificate, "tlsca", tlsca_name),
                ]
            ),
            "ORDERER_ADMIN_TLS_SIGN_CERT": _first_existing(
                [
                    _admin_candidate(admin_cert, "/tls/"),
                    os.path.join(admin_tls_dir, "client.crt"),
                    admin_cert,
                    os.path.join(runtime_orderer_dir, "tls", "server.crt"),
                    os.path.join(legacy_orderer_dir, "tls", "server.crt"),
                ]
            ),
            "ORDERER_ADMIN_TLS_PRIVATE_KEY": _first_existing(
                [
                    _admin_candidate(admin_key, "/tls/"),
                    os.path.join(admin_tls_dir, "client.key"),
                    admin_key,
                    os.path.join(runtime_orderer_dir, "tls", "server.key"),
                    os.path.join(legacy_orderer_dir, "tls", "server.key"),
                ]
            ),
        }

    elif node.type == "peer":
        peer_domain = f"{node.name}.{org_name}"
        legacy_peer_dir = os.path.join(dir_node, org_name, "peers", peer_domain)
        envs = {
            "CORE_PEER_TLS_ENABLED": "true",
            "CORE_PEER_LOCALMSPID": "{}MSP".format(
                org_name.split(".")[0].capitalize()
            ),
            "CORE_PEER_TLS_ROOTCERT_FILE": _first_existing(
                [
                    os.path.join(runtime_peer_dir, "tls", "ca.crt"),
                    os.path.join(legacy_peer_dir, "tls", "ca.crt"),
                ]
            ),
            "CORE_PEER_MSPCONFIGPATH": _first_existing(
                [
                    os.path.join(dir_node, org_name, "users", f"Admin@{org_name}", "msp"),
                    os.path.join(runtime_peer_dir, "msp"),
                ]
            ),
            "CORE_PEER_ADDRESS": "{}:{}".format(
                peer_domain, str(7051)
            ),
            "FABRIC_CFG_PATH": _first_existing(
                [
                    legacy_peer_dir,
                    runtime_peer_dir,
                ]
        ...
        if key in {
            ...
            if value and not os.path.exists(value):
                logging.getLogger(__name__).warning(
                    "Environment variable %s references missing path %s", key, value
                )
