...
# Refatoração: mover DisplayStrings para fora do Enum e ajustar metaclass
class EnumWithDisplayMeta(EnumMeta):
    def __new__(mcs, name, bases, attrs):
        # Buscar DisplayStrings externamente, se existir
        display_strings = None
        external_display_strings = attrs.get(f"_{name}__display_strings_class", None)
        if external_display_strings is not None:
            display_strings = external_display_strings
        elif "DisplayStrings" in attrs and inspect.isclass(attrs["DisplayStrings"]):
            display_strings = attrs["DisplayStrings"]
            del attrs["DisplayStrings"]

        obj = super().__new__(mcs, name, bases, attrs)
        for m in obj:
            m.display_string = getattr(display_strings, m.name, None) if display_strings else None
        return obj



# DisplayStrings externo para ErrorCode
class ErrorCodeDisplayStrings:
...
# Associar DisplayStrings externamente
setattr(ErrorCode, f"_{ErrorCode.__name__}__display_strings_class", ErrorCodeDisplayStrings)

def _errorcode_get_info(cls):
    error_code_str = """
    Error Codes:
    """
    for name, member in cls.__members__.items():
...
ErrorCode.get_info = classmethod(_errorcode_get_info)
