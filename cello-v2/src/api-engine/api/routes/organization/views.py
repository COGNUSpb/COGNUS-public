...
class OrganizationViewSet(viewsets.ViewSet):
    ...
    def create(self, request):
        ...
        if serializer.is_valid(raise_exception=True):
            ...
            try:
                ...
                LOG.warning(
                    "[GATEWAY_SYNC] Falha ao sincronizar gateway após criação da organização %s: %s",
                    organization.name,
                    exc,
                )
    ...
    def _conversion_msp_tls_cfg(self, type, org, node):
        ...
        try:
            if type == "peer":
                ...
                orderer_domain = org.split(".", 1)[1]
                orderer_tlsca_path = "{}/{}/crypto-config/ordererOrganizations/{}/tlsca/tlsca.{}-cert.pem".format(
                    CELLO_HOME, org, orderer_domain, orderer_domain
                )
                peer_tls_ca_path = "{}tls/ca.crt".format(dir_node)
                try:
                    with open(peer_tls_ca_path, "r", encoding="utf-8") as f_peer:
                        peer_tls_ca = f_peer.read()
                    with open(
                        orderer_tlsca_path, "r", encoding="utf-8"
                    ) as f_orderer:
                        orderer_tls_ca = f_orderer.read()
                    if orderer_tls_ca not in peer_tls_ca:
                        with open(
                            peer_tls_ca_path, "a", encoding="utf-8"
                        ) as f_peer:
                            if not peer_tls_ca.endswith("\n"):
                                f_peer.write("\n")
                            f_peer.write(orderer_tls_ca)
                except FileNotFoundError:
                    LOG.warning(
                        "Orderer TLS CA bundle missing for peer %s.%s",
                        node,
                        org,
                    )
                name = "core.yaml"
    ...
    def _generate_config(self, type, org, node):
        ...
        if type == "peer":
            couchdb_container = (
                "{}.{}".format(node, org).replace(".", "-") + "-cdb:5984"
            )
            ...
            args.update({"ledger_state_stateDatabase": "CouchDB"})
            args.update(
                {
                    "ledger_state_couchDBConfig_couchDBAddress": couchdb_container
                }
            )
            args.update(
                {
                    "ledger_state_couchDBConfig_username": couchdb_user,
                    "ledger_state_couchDBConfig_password": couchdb_password,
                }
            )
    ...
    def destroy(self, request, pk=None):
        ...
        try:
            ...
            try:
                ...
                LOG.warning(
                    "[GATEWAY_SYNC] Falha ao sincronizar gateway após remoção da organização: %s",
                    exc,
                )
        except ObjectDoesNotExist:
