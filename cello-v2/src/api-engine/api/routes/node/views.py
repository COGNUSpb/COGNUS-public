...
class NodeViewSet(viewsets.ViewSet):
    @staticmethod
    def _ca_signs_certificate(ca_path, cert_path):
        """Return True if cert_path is signed by ca_path, False otherwise."""
        if not (ca_path and cert_path):
            return False
        if not (os.path.exists(ca_path) and os.path.exists(cert_path)):
            return False
        try:
            subprocess.run(
    ...
    def _fix_orderer_ca_cert(self, org_name, node_name):
        """
        Garante que o arquivo ca.crt do diretório tls do orderer seja sempre sobrescrito corretamente
        com o certificado tlsca após a criação do orderer. Adiciona logs para facilitar o diagnóstico.
        """
        import glob
        import time
        orderer_short = org_name.split('.', 1)[1]
        orderer_dir = f"{CELLO_HOME}/{org_name}/crypto-config/ordererOrganizations/{orderer_short}/orderers/{node_name}.{orderer_short}/"
        ca_dir = f"{CELLO_HOME}/{org_name}/crypto-config/ordererOrganizations/{orderer_short}/msp/tlscacerts/"
        tls_dir = orderer_dir + "tls/"
        server_cert = tls_dir + "server.crt"
        ca_dst = tls_dir + "ca.crt"
        tentativas = 0
        while tentativas < 5:
            ca_files = glob.glob(ca_dir + "*.pem")
            if ca_files and os.path.exists(tls_dir):
                ca_src = ca_files[0]
                try:
                    if os.path.exists(server_cert) and not self._ca_signs_certificate(ca_src, server_cert):
                        LOG.warning(
                            "[CA_FIX] CA %s não assina o certificado %s; mantendo ca.crt atual para %s",
                            ca_src,
                            server_cert,
                            node_name,
                        )
                        return
                    shutil.copyfile(ca_src, ca_dst)
                    LOG.info(f"[CA_FIX] ca.crt sobrescrito: {ca_src} -> {ca_dst}")
                    break
                except Exception as e:
                    LOG.warning(f"[CA_FIX] Falha ao copiar CA cert para orderer: {e}")
            else:
                LOG.warning(f"[CA_FIX] Aguardando arquivos: tlsca em {ca_dir} ou tls em {tls_dir} para orderer {node_name}")
            tentativas += 1
            time.sleep(1)
        else:
            LOG.error(f"[CA_FIX] Não foi possível corrigir o ca.crt do orderer {node_name} após 5 tentativas.")

    def _refresh_tls_snapshot(self, node):
        """Regenera o payload TLS baseado no diretório atual do disco e atualiza o modelo se necessário."""
        try:
            ...
            if node.type == "orderer":
                ...
                runtime_dir = os.path.join(
                    ...
                    "hyperledger",
                    "fabric",
                    f"{node.name}.{org_domain}",
                    "tls",
                )
                static_dir = (
                    f"{CELLO_HOME}/{org_name}/crypto-config/ordererOrganizations/"
                    f"{org_domain}/orderers/{node.name}.{org_domain}/tls"
                )
                tls_candidates.extend([runtime_dir, static_dir])
            elif node.type == "peer":
                runtime_dir = os.path.join(
                    CELLO_HOME,
                    "hyperledger",
                    "fabric",
                    f"{node.name}.{org_name}",
                    "tls",
                )
                static_dir = (
                    f"{CELLO_HOME}/{org_name}/crypto-config/peerOrganizations/"
                    f"{org_name}/peers/{node.name}.{org_name}/tls"
                )
                tls_candidates.extend([runtime_dir, static_dir])

            tls_dir = next((d for d in tls_candidates if d and os.path.isdir(d)), None)
            if not tls_dir:
                LOG.debug(
                    "[TLS_SYNC] Diretório TLS inexistente para o node %s (candidatos: %s)",
                    node.name,
                    tls_candidates,
                )
                return
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                archive_path = os.path.join(tmpdir, "tls.zip")
                zip_dir(tls_dir, archive_path)
                with open(archive_path, "rb") as f_tls:
                    encoded_bytes = base64.b64encode(f_tls.read())

            encoded_repr = str(encoded_bytes)
            current_value = str(node.tls) if node.tls is not None else ""
            if encoded_repr != current_value:
                node.tls = encoded_bytes
                node.save(update_fields=["tls"])
                node.refresh_from_db(fields=["tls"])
                LOG.info(
                    "[TLS_SYNC] TLS bundle do node %s atualizado a partir de %s",
                    node.name,
                    tls_dir,
                )
        except Exception:
            LOG.exception(
                "[TLS_SYNC] Falha ao atualizar o bundle TLS do node %s",
                getattr(node, "name", "<desconhecido>"),
            )
    permission_classes = [
        IsAuthenticated,
    ]
    ...
    def create(self, request):
        ...
        try:
            ...
            if serializer.is_valid(raise_exception=True):
                ...
                # --- AJUSTE AUTOMÁTICO DO CA PARA ORDERER ---
                if node_type == "orderer":
                    self._fix_orderer_ca_cert(organization.name, node_name)
                ...
                # Ajuste para usar o admin cert/key no join do canal (apenas para orderer)
                if node_type == "orderer":
                    orderer_domain = organization.name.split(".", 1)[1]
                    tls_base = (
                        f"/opt/cello/{organization.name}/crypto-config/"
                        f"ordererOrganizations/{orderer_domain}/users/"
                        f"Admin@{orderer_domain}/tls"
                    )
                    node.admin_cert = f"{tls_base}/client.crt"
                    node.admin_key = f"{tls_base}/client.key"
                node.save()
    ...
    def _generate_config(self, type, org, node):
        ...
        if type == "peer":
            ...
            args.update(
                {
                    "peer_localMspId": org.split(".", 1)[0].capitalize()
                    + "MSP"
                }
            )
    ...
    def _agent_params(self, pk):
        ...
        try:
            ...
            self._refresh_tls_snapshot(node)
            network = org.network
            agent = org.agent.get()
            if agent is None:
            ...
            info["database"] = None if network is None else network.database
