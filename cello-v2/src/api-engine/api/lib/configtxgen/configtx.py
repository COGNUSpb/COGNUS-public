...
class ConfigTX:
    ...
    def create(
        ...
        runtime_fabric_base = os.path.join(CELLO_HOME, "hyperledger", "fabric")
        ...
        for orderer in orderers:
            ...
            OrdererOrg = dict(
                Name="Orderer",
                ID=OrdererMSP,
                MSPDir=_existing_path(
                    "{}/{}/crypto-config/ordererOrganizations/{}/msp".format(
                        self.filepath,
                        orderer["name"],
                        orderer["name"].split(".", 1)[1],
                    ),
                    os.path.join(runtime_fabric_base, "{}.{}".format(
                        orderer["hosts"][0]["name"], orderer["name"].split(".", 1)[1]
                    ), "msp"),
                ),
                Policies=dict(
                    Readers=dict(
            ...
            for host in orderer["hosts"]:
                ...
                domain = orderer["name"].split(".", 1)[1]
                runtime_orderer_dir = os.path.join(
                    runtime_fabric_base, f"{host['name']}.{domain}"
                )
                legacy_orderer_dir = "{}/{}/crypto-config/ordererOrganizations/{}/orderers/{}.{}/tls".format(
                    self.filepath,
                    orderer["name"],
                    domain,
                    host["name"],
                ...
                Consenters.append(
                    dict(
                        Host="{}.{}".format(host["name"], domain),
                        Port=7050,
                        ClientTLSCert=_existing_path(
                            os.path.join(runtime_orderer_dir, "tls", "server.crt"),
                            os.path.join(legacy_orderer_dir, "server.crt"),
                        ),
                        ServerTLSCert=_existing_path(
                            os.path.join(runtime_orderer_dir, "tls", "server.crt"),
                            os.path.join(legacy_orderer_dir, "server.crt"),
                        ),
                    )
        ...
        for peer in peers:
            ...
            runtime_peer_dir = os.path.join(
                runtime_fabric_base, f"{peer['hosts'][0]['name']}.{peer['name']}"
            )
            legacy_peer_dir = "{}/{}/crypto-config/peerOrganizations/{}/msp".format(
            ...
            PeerOrganizations.append(
                dict(
                    ...
                    ID=PeerMSP,
                    MSPDir=_existing_path(legacy_peer_dir, runtime_peer_dir),
                    Policies=dict(
                        Readers=dict(
                            Type="Signature",
