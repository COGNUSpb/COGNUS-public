...
class ChainCode(Command):
    def __init__(self, version=FABRIC_VERSION, peer=FABRIC_TOOL, **kwargs):
        self.peer = peer + "/peer"
        # Sanitize env vars and keep a local copy for later resolution.
        sanitized_env = {
            key: str(value) for key, value in kwargs.items() if value is not None
        }
        self._env = sanitized_env
        super(ChainCode, self).__init__(version, **sanitized_env)

    def _get_orderer_ca(self):
        """Return the path to the orderer TLS CA, if available."""
        orderer_ca = os.getenv("ORDERER_CA") or self._env.get("ORDERER_CA")
        if orderer_ca:
            return orderer_ca

        admin_cert_path = self._env.get("ORDERER_ADMIN_TLS_SIGN_CERT")
        if admin_cert_path:
            tls_ca_dir = (
                Path(admin_cert_path).expanduser().resolve().parent.parent
                / "msp"
                / "tlscacerts"
            )
            if tls_ca_dir.exists():
                for candidate in tls_ca_dir.glob("*.pem"):
    ...
    def lifecycle_install(self, cc_targz):
        ...
        try:
            ...
            already_installed = (
                "already successfully installed" in stderr.lower()
                if stderr
                else False
            )
            if proc.returncode != 0 and already_installed:
                LOG.info(
                    "Chaincode package %s already installed: %s",
                    cc_targz,
                    stderr.strip(),
                )
    ...
    def lifecycle_approve_for_my_org(
        ...
        try:
            ...
            command = []
            tls_flag = os.getenv("CORE_PEER_TLS_ENABLED")
            if tls_flag == "false" or tls_flag is None:
                command = [
                    self.peer,
                    "lifecycle",
                    "chaincode",
                ...
                orderer_ca = self._get_orderer_ca()
                if not orderer_ca:
                    raise Exception(
                        "Unable to resolve ORDERER_CA (orderer TLS CA path)"
                    )
                command = [
                    self.peer,
                    "lifecycle",
